#!/usr/bin/env python3
"""
VILONA FINAL BACKTEST - USE LITERAL PATH WITH ESCAPING
Working path (as stored in filesystem):
...
# Use literal path - need to be careful with special chars
# The directory literally contains backslashes in the name on this system!
TRADING_PATH = r"/home/openclaw/C:\Users\EX PC\.openclaw\workspace/skills/1ai-skills\trading"
...
# Now run the scripts with relative paths
STRATEGIES_TO_TEST = [
...
print("="*80)
print("VILONA FINAL BACKTEST - LITERAL PATH MODE")
print("="*80)
print()
...
if results:
    ...
    for r in sorted(results, key=lambda x: -x.get("wr", 0)):
        print(f"{r['pair']:<8} {r['tf']:>3} WR: {r['wr']:>5.1f}% PNL: ${r.get('pnl', 0):>7.2f}")
