#!/usr/bin/env python3
"""
Add Backtest Methods to Template Strategies
Simple backtest implementations for Holy Grail, Kumo, Momentum Elder, Volume Momentum
"""
...
# Define template formatter
BACKTEST_METHOD_TEMPLATE = BACKTEST_METHOD_TEMPLATE

STRATEGIES = []
STRATEGIES.append({
    'file': f"{TRADING_DIR}/strategy/templates/forex/holy_grail.py",
    'class_name': 'HolyGrailStrategy',
    'strategy_name': 'Holy Grail'
})
STRATEGIES.append({
    'file': f"{TRADING_DIR}/strategy/templates/forex/kumo_breakout.py",
    'class_name': 'KumoBreakoutStrategy',
    'strategy_name': 'Kumo Breakout'
})
STRATEGIES.append({
    'file': f"{TRADING_DIR}/strategy/templates/forex/momentum_elder.py",
    'class_name': 'MomentumElderStrategy',
    'strategy_name': 'Momentum Elder'
})
STRATEGIES.append({
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Add Backtest Methods to Template Strategies')
    parser.add_argument('--strategy', choices=['all', 'holy-grail', 'kumo', 'momentum-elder', 'volume-momentum'], default='all', help='Strategy to update')

    args = parser.parse_args()

    strategies_to_update = STRATEGIES
    if args.strategy != 'all':
        strategies_to_update = [s for s in STRATEGIES if s['strategy_name'].lower().replace(' ', '-') == args.strategy]
    ...
    for strategy_info in strategies_to_update:
        ...
        backtest_method = BACKTEST_METHOD_TEMPLATE.replace('{0}', strategy_name)

        # Find class end - look for the last method in the class
        import re
        class_end_pattern = r'(\n    def [a-z_]+\('
        matches = list(re.finditer(class_end_pattern, content))

        if matches:
            # Insert backtest method after the last method
            ...
            print(f"❌ Could not find insert point for {strategy_info['strategy_name']}")
