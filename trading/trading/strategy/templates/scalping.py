...
class ScalpingTemplate(StrategyTemplate):
    ...
    def __init__(
        ...
        ema_medium: int = 13,
        ema_slow: int = 50,
        risk_per_trade: float = 0.01,  # Lower risk for scalping
    ...
    def calculate_ema_stack(
        ...
        Returns:
            (ema_fast, ema_medium, ema_slow, trend)
    ...
    def entry_conditions(
        ...
        Entry signals:
        - Bullish: EMAs aligned upward + positive momentum
        - Bearish: EMAs aligned downward + negative momentum
        """
        if current_idx < self.ema_slow:
            return False, None
        ...
        # Bullish entry: EMA alignment + positive momentum
        ...
        # Bearish entry: EMA alignment + negative momentum
    ...
    def exit_conditions(
        ...
        if position.side == "LONG":
            ...
            # Exit if EMA alignment breaks (trend change)
            if trend != "bullish":
                return True, Signal(
                    timestamp=current.time,
                    symbol=self.symbol,
