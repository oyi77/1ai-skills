#!/usr/bin/env python3
"""
MOMENTUM ELDER STRATEGY - Simplified Version
Elder-Ray Index-based momentum strategy
"""
...
def calculate_elder_ray(df, ema_period=13):
...
def momentum_elder_strategy(df, initial_balance=100, min_momentum=0.0001, rr_ratio=2.0):
    """
    Momentum Elder Strategy:
    - Entry: Bull Power > 0 and increasing (long), Bear Power < 0 and decreasing (short)
    - Filter: Only trade if momentum >= min_momentum
    - Exit: TP = Entry + (Momentum × RR), SL = Entry - Momentum
    ...
    for i in range(2, len(df)):
        ...
        # Current momentum
        bull_power = current_candle['bull_power']
        bear_power = current_candle['bear_power']

        # Momentum change (trend strength)
        bull_change = bull_power - prev_candle['bull_power']
        bear_change = bear_power - prev_candle['bear_power']

        # Filter: Minimum momentum
        if abs(bull_power) < min_momentum and abs(bear_power) < min_momentum:
            continue

        # Calculate risk amount (1% of balance)
        risk_amount = balance * 0.01

        # Check for long entry (bullish momentum)
        if bull_power > 0 and bull_change > 0:
            entry = current_candle['Close']
            stop_loss = prev_candle['Low']  # Below previous low
            momentum_abs = abs(bull_power)
            take_profit = entry + (momentum_abs * rr_ratio)
        ...
        # Check for short entry (bearish momentum)
        elif bear_power < 0 and bear_change < 0:
            entry = current_candle['Close']
            stop_loss = prev_candle['High']  # Above previous high
            momentum_abs = abs(bear_power)
            take_profit = entry - (momentum_abs * rr_ratio)
            risk_per_lot = abs(entry - stop_loss)
...
def main():
    parser = argparse.ArgumentParser(description='Momentum Elder Strategy Backtest')
    ...
    parser.add_argument('--min-momentum', type=float, default=0.0001, help='Minimum momentum')
    ...
    if args.action == 'backtest':
        print("="*80)
        print("MOMENTUM ELDER STRATEGY BACKTEST")
        print("="*80)
        ...
        results = momentum_elder_strategy(
            df=df,
        ...
        # Save results
        output_file = "/tmp/momentum_elder_backtest.json"
