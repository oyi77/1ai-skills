...
def main():
    ...
    parser.add_argument(
        "--top", action="store_true", help="Show top markets by 24h volume"
    )
    ...
    if args.json:
        if args.search:
            ...
            data = [
                m for m in data if args.search.lower() in m.get("question", "").lower()
            ][: args.limit]
        elif args.slug:
            data = fetch("/markets", {"slug": args.slug})
        elif args.events:
            data = fetch(
                "/events", {"active": "true", "closed": "false", "limit": args.limit}
            )
        else:
            data = fetch(
                "/markets",
