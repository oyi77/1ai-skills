#!/usr/bin/env python3
"""
Viral Video Pipeline — Complete with video hosting (api.video)
Generate videos → Upload to api.video → Schedule to all accounts
"""
...
# ── Config ────────────────────────────────────────────────────────────────────
# api.video free tier: 10GB storage, 500 uploads/month
API_VIDEO_KEY = "YOUR_API_VIDEO_KEY"  # User needs to add this
POST_BRIDGE_KEY  = "pb_live_Kyc2gafDF7Qc8c2ALELtEC"

OUTPUT_DIR = Path("output/viral_complete")
...
def create_video(img_path: Path, output_path: Path) -> bool:
    """Create 15s video with zoom effect."""
...
def upload_to_video_hosting(video_path: Path) -> str:
    """Upload video to hosting service and return public URL."""
    print(f"  📤 Uploading video...")
    
    # For now, just return placeholder
    # User needs to add API_VIDEO_KEY for api.video
    
    if API_VIDEO_KEY == "YOUR_API_VIDEO_KEY":
        print(f"  ⚠️  No video hosting API key set")
        print(f"     Get free key at: https://api.video/")
        return ""
    ...
    req.add_header("Authorization", f"Bearer {API_VIDEO_KEY}")
    ...
    try:
        ...
        video_url = result.get("assets", {}).get("mp4", "")
        print(f"  ✅ Uploaded: {video_url[:60]}...")
        return video_url
...
def main():
    print("=" * 70)
    print("🚀 VIRAL VIDEO PIPELINE — COMPLETE WITH VIDEO HOSTING")
    print("=" * 70)
    print()
    ...
    if API_VIDEO_KEY == "YOUR_API_VIDEO_KEY":
        ...
        print("=" * 70)
        print()
        print("Video hosting API key not configured.")
        print()
        print("Get a FREE api.video key:")
        print("  1. Go to: https://api.video/register")
        print("  2. Sign up (free tier: 10GB storage)")
        print("  3. Get API key from dashboard")
        print("  4. Add to script: API_VIDEO_KEY = 'your-key-here'")
        print()
        print("Then run again:")
        print("  python3 viral_complete_pipeline.py")
        print()
        print("=" * 70)
        return
    ...
    for i, concept in enumerate(CONCEPTS):
        ...
        if not vid_path.exists():
            ok = create_video(img_path, vid_path)
            if not ok:
                print(f"  ⚠️  Skipping")
                continue
        else:
        ...
        # Upload to video hosting
        video_url = upload_to_video_hosting(vid_path)
        if not video_url:
        ...
        post_fb = schedule_post(fb_ids, concept["caption"], video_url, sched_utc)
        post_tik = schedule_post(tik_ids, concept["caption"], video_url, sched_utc)
