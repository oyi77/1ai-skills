#!/usr/bin/env python3
"""
Viral Content Simple Pipeline — Create + Schedule 5 viral posts
Uses FFmpeg to create images with text overlays, posts to all accounts.
"""
...
OUTPUT_DIR = Path("output/viral_simple")
...
def create_image(concept: dict, output_path: Path) -> bool:
    """Create image with gradient background and text overlay."""
    print(f"  🎨 Creating image with FFmpeg...")
    
    bg_color = concept["bg_color"]
    text_color = concept["text_color"]
    headline = concept["headline"]
    
    # Create gradient background and add text
    cmd = [
        "ffmpeg", "-y",
        "-f", "lavfi",
        "-i", f"color=c={bg_color}:s=1080x1920:d=1",
        "-vf", (
            f"drawtext=text='{headline.replace(chr(10), '\\n')}':"
            f"fontsize=68:fontcolor={text_color}:"
...
def schedule_post(account_ids: list, caption: str, media_url: str, sched_time: str) -> dict:
    ...
    try:
        ...
        return {"error": str(e), "body": e.read().decode("utf-8")[:200]}
    except Exception as e:
        return {"error": str(e)}
...
def main():
    print("=" * 60)
    print("🚀 VIRAL CONTENT SIMPLE PIPELINE")
    print("=" * 60)
    print()
    ...
    for i, concept in enumerate(CONCEPTS):
        ...
        if not img_path.exists():
            ok = create_image(concept, img_path)
            if not ok:
