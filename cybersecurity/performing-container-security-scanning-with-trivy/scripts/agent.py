...
def analyze_vulnerabilities(scan_results):
    ...
    return {
        ...
        "critical_high_cves": sorted(
            cve_list, key=lambda x: x.get("cvss_score", 0), reverse=True
        )[:20],
    }
...
def main():
    parser = argparse.ArgumentParser(
        description="Trivy Container Security Scanning Agent"
    )
    parser.add_argument(
        "--image", required=True, help="Container image to scan (e.g., nginx:latest)"
    )
    parser.add_argument(
        "--config-path", help="Path to Dockerfile/K8s manifests for misconfig scan"
    )
    parser.add_argument(
        "--severity", default="CRITICAL,HIGH,MEDIUM", help="Severity filter"
    )
    ...
    report = generate_report(
        args.image, vuln_analysis, secret_analysis, misconfig_analysis
    )
