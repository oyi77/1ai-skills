...
def enumerate_domain_users(
    domain: str, dc_ip: str, user: str, password: str, output_dir: Path
) -> list[str]:
...
def generate_report(domain: str, output_dir: Path) -> str:
    ...
    if kf.exists():
        with open(kf) as f:
            kerberoast_count = sum(
                1 for line in f if line.strip() and line.startswith("$krb5tgs$")
            )
    ...
    if af.exists():
        with open(af) as f:
            asrep_count = sum(
                1 for line in f if line.strip() and line.startswith("$krb5asrep$")
            )
    ...
    with open(report_file, "w") as f:
        ...
        f.write(
            "1. Convert service accounts to Group Managed Service Accounts (gMSA)\n"
        )
...
def main():
    ...
    users = enumerate_domain_users(
        args.domain, args.dc_ip, args.username, args.password, output_dir
    )
