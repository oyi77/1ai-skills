...
def detect_webshell_process(event):
    ...
    if eid != "1":
        return None
    parent = (
        event.get("ParentImage", event.get("InitiatingProcessFileName", ""))
    ).lower()
    image = (event.get("Image", event.get("FileName", ""))).lower()
    parent_name = parent.split("\\")[-1].split("/")[-1]
    image_name = image.split("\\")[-1].split("/")[-1]
    if parent_name not in WEB_SERVER_PROCESSES:
        return None
    if image_name not in SUSPICIOUS_CHILDREN:
        return None
    return {
        "detection_type": "WEBSHELL_PROCESS_SPAWN",
        "technique": "T1505.003",
        "web_server": parent_name,
        "child_process": image_name,
...
def detect_webshell_file(event):
    ...
    if not any(re.search(p, target, re.IGNORECASE) for p in WEB_DIRECTORIES):
        return None
    ext = "." + target.split(".")[-1] if "." in target else ""
    if ext not in WEBSHELL_EXTENSIONS:
        return None
    return {
        ...
        "file_path": target,
        "extension": ext,
        "creating_process": event.get(
            "Image", event.get("InitiatingProcessFileName", "")
        ),
        "hostname": event.get("Computer", event.get("DeviceName", "unknown")),
        "timestamp": event.get("UtcTime", event.get("Timestamp", "")),
        "risk_score": 70,
...
def run_hunt(input_path, output_dir):
    ...
    with open(Path(output_dir) / "webshell_findings.json", "w", encoding="utf-8") as f:
        json.dump(
            {"hunt_id": f"TH-WEBSHELL-{datetime.date.today()}", "findings": findings},
            f,
            indent=2,
        )
...
def main():
    ...
    h.add_argument("--output", "-o", default="./webshell_output")
