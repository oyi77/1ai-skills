...
class DuoMFAAuditor:
    ...
    def _audit_method_distribution(self):
        ...
        if total > 0:
            weak_pct = (
                sum(method_counts.get(m, 0) for m in self.WEAK_METHODS) / total * 100
            )
            if weak_pct > 10:
                self.findings.append(
                    MFAAuditFinding(
    ...
    def generate_report(self) -> str:
        ...
        lines.append(
            f"OVERALL: {'FAIL' if critical > 0 else 'PASS WITH FINDINGS' if self.findings else 'PASS'}"
        )
        lines.append("=" * 70)
        return "\n".join(lines)
...
def main():
    ...
    auditor.load_policies(
        [
            {
                "policy_name": "Standard Users",
                "group": "standard_users",
                "allowed_methods": ["push", "totp", "sms"],
                "remembered_devices_days": 7,
                "failmode": "secure",
            },
            {
                "policy_name": "Privileged Admins",
                "group": "privileged_admins",
                "allowed_methods": ["push", "totp"],
                "remembered_devices_days": 0,
                "require_device_health": True,
                "failmode": "safe",
            },
            {
