...
PURDUE_ZONES = {
    ...
    "level_2": {
        "name": "Supervisory",
        "description": "HMI, SCADA, engineering workstations",
    },
    ...
    try:
        ...
        for rule in rules:
            if rule.get("action") == "permit":
                ...
                if src_zone in ("enterprise", "external") and dst_zone in (
                    "control",
                    "process",
                ):
                    findings.append(
        ...
        findings.append(
            {"issue": "Firewall config not found or invalid", "severity": "critical"}
        )
    return findings
...
def audit_zone_compliance(zone_config):
    ...
    for zone_id, zone_data in zone_config.items():
        if zone_id not in PURDUE_ZONES:
            findings.append(
                {"zone": zone_id, "issue": "Non-standard zone", "severity": "medium"}
            )
            continue
                ...
                findings.append(
                    {
                        "zone": zone_id,
                        "host": host.get("ip"),
                        "issue": "Workstation in control/process zone",
                        "severity": "high",
                    }
                )
            if host.get("internet_access") and zone_id in (
                "level_0",
                "level_1",
                "level_2",
            ):
                findings.append(
                    {
                        "zone": zone_id,
                        "host": host.get("ip"),
                        "issue": "Internet access from OT zone",
                        "severity": "critical",
                    }
                )
    return findings
...
def main():
    ...
    fw_findings = (
        check_firewall_rules(args.firewall_config) if args.firewall_config else []
    )
    ot_exposure = (
        check_ot_protocol_exposure(args.scan_subnet) if args.scan_subnet else []
    )
    zone_config = {}
    zone_findings = []
    if args.zone_config:
    ...
    logger.info(
        "OT segmentation: %d findings (%d critical), %d exposed OT ports",
