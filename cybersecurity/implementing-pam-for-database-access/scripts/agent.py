        ...
        for pattern in shared_patterns:
            if pattern in raw:
                findings.append(
                    {
                        "user": raw,
                        "issue": f"Potential shared account ({pattern})",
                        "severity": "high",
                    }
                )
    return findings
...
def generate_report(users, shared, logging_findings, tls):
    ...
    return {
        ...
        "shared_account_findings": shared,
...
def main():
    ...
    parser.add_argument(
        "--db-type", required=True, choices=["postgresql", "mysql", "mssql"]
    )
    ...
    users = query_db_users(
        args.db_type, args.host, args.admin_user, args.admin_password
    )
    shared = audit_shared_accounts(users)
    logging_f = audit_session_logging(
        args.db_type, args.host, args.admin_user, args.admin_password
    )
    ...
    logger.info(
        "PAM DB audit: %d users, %d findings", len(users), report["total_findings"]
    )
