...
def load_netflow_csv(filepath):
    ...
    if missing:
        ...
        df.rename(
            columns={k: v for k, v in alt_map.items() if k in df.columns}, inplace=True
        )
...
def compute_hourly_baseline(df):
    ...
    hourly["bytes_mean"] = hourly["total_bytes"] / max(
        df["timestamp"].dt.date.nunique(), 1
    )
...
def detect_zscore_anomalies(df, host_baselines, threshold=3.0):
    ...
    if std_bytes == 0:
        return []
    host_baselines["zscore"] = (
        (host_baselines["total_bytes"] - mean_bytes) / std_bytes
    ).round(4)
...
def detect_iqr_anomalies(df, host_baselines):
    ...
    outliers = host_baselines[
        (host_baselines["mean_bytes_per_flow"] < lower)
        | (host_baselines["mean_bytes_per_flow"] > upper)
    ]
    alerts = []
...
def main():
    ...
    parser.add_argument(
        "--netflow-csv", required=True, help="Path to NetFlow/IPFIX CSV export"
    )
    parser.add_argument(
        "--zscore-threshold", type=float, default=3.0, help="Z-score anomaly threshold"
    )
    parser.add_argument(
        "--scan-threshold",
        type=int,
        default=50,
        help="Port scan unique ports threshold",
    )
    parser.add_argument(
        "--output", default="traffic_baseline_report.json", help="Output report path"
    )
    ...
    top_talkers = host_baselines.nlargest(10, "total_bytes")[
        ["src_ip", "total_bytes", "flow_count"]
