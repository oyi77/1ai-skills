    ...
    return {
        ...
        "maturity_level": (
            ...
            if score >= 90
                ...
                "advanced"
                if score >= 70
                else "initial" if score >= 50 else "traditional"
            )
        ),
    }
...
def assess_conditional_access(policies: list) -> dict:
    ...
    for policy in policies:
        ...
        if conditions.get("locations") and conditions["locations"].get(
            "excluded_trusted"
        ):
            coverage["location_restrictions"] = True

        if session_controls.get("sign_in_frequency") or session_controls.get(
            "persistent_browser"
        ):
        ...
        if (
            conditions.get("roles")
            and "privileged" in str(conditions.get("roles", [])).lower()
        ):
...
def generate_identity_maturity_report(config: dict) -> dict:
    ...
    ca_assessment = assess_conditional_access(
        config.get("conditional_access_policies", [])
    )
    ...
    report["overall_maturity"] = (
        ...
        if avg_score >= 90
            ...
            "advanced"
            if avg_score >= 70
            else "initial" if avg_score >= 50 else "traditional"
        )
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Identity Verification Assessment Tool"
    )
    parser.add_argument(
        "--config", type=str, help="Path to identity configuration JSON"
    )
    parser.add_argument("--logs", type=str, help="Path to sign-in logs JSON")
    parser.add_argument(
        "--action",
        choices=["assess", "analyze-logs", "detect-travel", "report"],
        default="report",
    )
    ...
    if args.action == "assess" and args.config:
        ...
        ca_result = assess_conditional_access(
            config.get("conditional_access_policies", [])
        )
        ...
        print(
            f"MFA Score: {mfa_result['mfa_score']}, CA Coverage: {ca_result['coverage_score']}%"
        )
        ...
        print(
            f"Analyzed {stats['total_sign_ins']} sign-ins, {len(stats['risky_sign_ins'])} risky"
        )
        ...
        print(
            f"Identity Maturity: {report['overall_maturity']} (Score: {report['overall_score']})"
        )
