...
def detect_null_sid_logons(events):
    ...
    for event in events:
        ...
        if (sid == "S-1-0-0" or sid == "NULL SID" or not sid) and (
            not guid or guid == "{00000000-0000-0000-0000-000000000000}"
        ):
...
def check_smb_signing(hosts=None):
    ...
    try:
        ...
        if proc.returncode != 0:
            return [
                {"error": "Failed to enumerate hosts", "stderr": proc.stderr.strip()}
            ]
        ...
        try:
            ...
            if proc.stdout.strip():
                ...
                if not result.get("RequireSecuritySignature", True):
                    ...
                    result["message"] = (
                        f"SMB signing not required on {host} — vulnerable to relay"
                    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Hunt for NTLM relay attacks in Windows event logs"
    )
    parser.add_argument(
        "--max-events",
        type=int,
        default=2000,
        help="Max events to query (default: 2000)",
    )
    parser.add_argument(
        "--check-pipes",
        action="store_true",
        help="Also check Event 5145 for named pipe access",
    )
    parser.add_argument(
        "--check-smb-signing",
        action="store_true",
        help="Audit SMB signing on domain hosts",
    )
    parser.add_argument(
        "--hosts", help="Comma-separated list of hosts to check SMB signing"
    )
    parser.add_argument(
