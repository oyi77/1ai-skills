...
def generate_report(tls_findings, beacon_findings, http_findings):
    ...
    return {
        ...
        "top_suspect_destinations": dict(
            sorted(by_dst.items(), key=lambda x: x[1], reverse=True)[:10]
        ),
...
def main():
    ...
    parser.add_argument(
        "--zeek-dir", required=True, help="Directory containing Zeek log files"
    )
    parser.add_argument(
        "--min-connections",
        type=int,
        default=20,
        help="Minimum connections for beacon analysis",
    )
    parser.add_argument(
        "--max-jitter",
        type=int,
        default=25,
        help="Maximum jitter percentage for beacon scoring",
    )
    ...
    beacon_findings = analyze_beacon_timing(
        conn_log, args.min_connections, args.max_jitter
    )
    ...
    logger.info(
        "CS Hunt: %d TLS hits, %d beacons, %d HTTP matches, CS likely: %s",
        len(tls_findings),
        len(beacon_findings),
        len(http_findings),
        report["cobalt_strike_likely"],
    )
