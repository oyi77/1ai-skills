...
def parse_profile_regex(content):
    ...
    block_pattern = re.compile(
        r"(http-get|http-post|http-stager|https-certificate|dns-beacon|process-inject|post-ex)\s*\{",
        re.MULTILINE,
    )
    ...
    for match in header_pattern.finditer(content):
        config.setdefault("headers", []).append(
...
def generate_suricata_rules(findings, sid_start=9000001):
    ...
    for f in findings:
        if f["type"] == "c2_uri" and f.get("indicator"):
            ...
            pipe = f["indicator"]
            rules.append(f"# Named pipe detection requires endpoint monitoring: {pipe}")
    return rules
...
def main():
    ap = argparse.ArgumentParser(
        description="CobaltStrike Malleable C2 Profile Analyzer"
    )
    ap.add_argument(
        "--input", required=True, help="Input .profile file or JSON with parsed config"
    )
