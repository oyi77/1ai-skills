...
def analyze(alerts: list) -> dict:
    return {
        ...
        "override_rate": round(
            sum(1 for a in alerts if a["overridden"]) / max(len(alerts), 1) * 100, 2
        ),
...
if __name__ == "__main__":
    ...
    with open(out, "w") as f:
        json.dump(
            {"report_generated": datetime.utcnow().isoformat() + "Z", **result},
            f,
            indent=2,
        )
    print(
        f"Total: {result['total']} | Blocked: {result['blocked']} | Override rate: {result['override_rate']}%"
    )
