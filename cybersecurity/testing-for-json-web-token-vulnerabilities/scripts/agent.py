    ...
    def analyze_token(self, token):
        ...
        if alg in ("HS256", "HS384", "HS512"):
            issues.append(
                {
                    "severity": "info",
                    "issue": f"Symmetric {alg} - test for weak secrets",
                }
            )
        if "kid" in header:
            issues.append(
                {
                    "severity": "info",
                    "issue": f"kid present: {header['kid']} - test for injection",
                }
            )
        if "jku" in header:
            issues.append(
                {
                    "severity": "medium",
                    "issue": f"jku present: {header['jku']} - test JWKS spoofing",
                }
            )
    ...
    def forge_token(self, token, claims_override, secret=None):
        ...
        if secret and header.get("alg") in ("HS256", "HS384", "HS512"):
            ...
            sig = (
                base64.urlsafe_b64encode(
                    hmac.new(
                        secret.encode(), signing_input, alg_map[header["alg"]]
                    ).digest()
                )
                .decode()
                .rstrip("=")
