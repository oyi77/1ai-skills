...
class SplunkSOARClient:
    ...
    def _get(self, endpoint, params=None):
        resp = self.session.get(
            f"{self.base_url}/rest{endpoint}", params=params, timeout=30
        )
    ...
    def _post(self, endpoint, data=None):
        resp = self.session.post(
            f"{self.base_url}/rest{endpoint}", json=data, timeout=30
        )
        resp.raise_for_status()
        return resp.json()
...
def run_soar_audit(client):
    ...
    for ct in ct_data[:10]:
        print(
            f"  [{ct.get('severity', 'N/A')}] {ct.get('name', 'N/A')} (Status: {ct.get('status')})"
        )
...
def main():
    ...
    if args.audit:
        ...
        result = client.run_playbook(
            int(args.run_playbook[0]), int(args.run_playbook[1])
        )
