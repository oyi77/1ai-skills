...
def audit_key_file(key_path):
    ...
    if not os.path.isfile(key_path):
        findings.append(
            {"check": "File exists", "status": "FAIL", "severity": "CRITICAL"}
        )
        return findings
    ...
    try:
        if b"PRIVATE" in pem_data:
            ...
            findings.append(
                {"check": "Key type", "status": "Public key", "severity": "INFO"}
            )
        ...
        if key_size < 2048:
            ...
            findings.append(
                {
                    "check": "Key strength",
                    "status": f"PASS ({key_size} bits)",
                    "severity": "INFO",
                }
            )
    except Exception as e:
        findings.append(
            {"check": "Key parsing", "status": f"FAIL: {e}", "severity": "HIGH"}
        )
    return findings
...
def main():
    parser = argparse.ArgumentParser(
        description="RSA key pair lifecycle management agent"
    )
    ...
    p_gen.add_argument(
        "--private-key", default="id_rsa", help="Private key output path"
    )
    p_gen.add_argument(
        "--public-key", default="id_rsa.pub", help="Public key output path"
    )
    ...
    p_audit.add_argument(
        "--recursive", action="store_true", help="Scan directory recursively"
    )
