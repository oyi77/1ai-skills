...
ALERT_CATEGORIES = {
    "malware": {
        "default_priority": "P2",
        "auto_escalate_if": ["ransomware", "wiper", "apt"],
    },
    "phishing": {
        "default_priority": "P3",
        "auto_escalate_if": ["executive_target", "credential_harvested"],
    },
    "unauthorized_access": {
        "default_priority": "P2",
        "auto_escalate_if": ["admin_account", "domain_controller"],
    },
    "data_exfiltration": {
        "default_priority": "P1",
        "auto_escalate_if": ["pii", "financial", "classified"],
    },
    "denial_of_service": {
        "default_priority": "P2",
        "auto_escalate_if": ["customer_facing", "revenue_impacting"],
    },
    "insider_threat": {
        "default_priority": "P2",
        "auto_escalate_if": ["privileged_user", "data_staging"],
    },
    "vulnerability_exploit": {
        "default_priority": "P2",
        "auto_escalate_if": ["zero_day", "active_exploitation"],
    },
}
...
def classify_alert(category, tags, affected_asset_criticality="medium"):
    ...
    cat_info = ALERT_CATEGORIES.get(
        category, {"default_priority": "P3", "auto_escalate_if": []}
    )
...
def build_escalation_matrix():
    ...
    matrix["auto_escalation_rules"] = [
        {
            "rule": "SLA breach: response",
            "action": "Escalate to next tier in chain",
            "condition": "Response SLA exceeded",
        },
        {
            "rule": "SLA breach: update",
            "action": "Notify SOC Lead",
            "condition": "Update SLA exceeded",
        },
        {
            "rule": "SLA breach: resolution",
            "action": "Escalate to IR Manager",
            "condition": "Resolution SLA exceeded",
        },
        {
            "rule": "Multiple related alerts",
            "action": "Escalate priority by 1",
            "condition": ">= 3 correlated alerts",
        },
        {
            "rule": "VIP user affected",
            "action": "Auto-escalate to P1",
            "condition": "Executive or board member",
        },
    ]
    return matrix
    ...
    for tier_id, tier in matrix["tiers"].items():
        ...
        if not tier.get("notification_channels"):
            issues.append(
                {
                    "tier": tier_id,
                    "issue": "No notification channels",
                    "severity": "high",
                }
            )
    for cat, info in matrix["categories"].items():
        if info["default_priority"] not in matrix["tiers"]:
            issues.append(
                {
                    "category": cat,
                    "issue": f"Invalid priority {info['default_priority']}",
                    "severity": "critical",
                }
            )
...
def generate_report(matrix, validation, simulation_results=None):
    ...
    print(
        f"ESCALATION MATRIX: {status}, {validation['tier_count']} tiers, "
