    ...
    payload = json.dumps(
        {
            ...
            "timestamp": timestamp,
            "nonce": hashlib.sha256(f"{time.time()}{source_ip}".encode()).hexdigest()[
                :16
            ],
        }
    ).encode()
...
def validate_spa_packet(
    packet: bytes, shared_key: str, max_age_seconds: int = 60
) -> dict:
    ...
    try:
        ...
        if age > max_age_seconds:
            result["errors"].append(
                f"Packet expired ({age:.0f}s old, max {max_age_seconds}s)"
            )
            return result
...
def validate_mtls_certificate(
    host: str, port: int, ca_cert_path: Optional[str] = None
) -> dict:
...
def validate_sdp_config(config: dict) -> dict:
    ...
    if not controller.get("idp_integration"):
        findings.append(
            {"severity": "critical", "finding": "No IdP integration configured"}
        )
        score -= 25

    if not controller.get("audit_logging"):
        findings.append(
            {"severity": "high", "finding": "Audit logging not enabled on controller"}
        )
        score -= 10
    ...
    for gw in gateways:
        ...
        if not gw.get("spa_enabled"):
            findings.append(
                {
                    "severity": "critical",
                    "finding": f"Gateway {gw.get('name')}: SPA not enabled",
                }
            )
            score -= 15
        if not gw.get("mtls_enabled"):
            findings.append(
                {
                    "severity": "high",
                    "finding": f"Gateway {gw.get('name')}: mTLS not configured",
                }
            )
            score -= 10
    ...
    if not pki.get("ocsp_enabled") and not pki.get("crl_enabled"):
        findings.append(
            {
                "severity": "high",
                "finding": "No certificate revocation checking enabled",
            }
        )
        score -= 10
    ...
    if not monitoring.get("siem_integration"):
        findings.append(
            {"severity": "warning", "finding": "No SIEM integration for SDP events"}
        )
        score -= 5
    ...
    return {
        ...
        "findings": findings,
        "status": (
            "ready" if score >= 80 else "needs_work" if score >= 50 else "not_ready"
        ),
        "timestamp": datetime.now().isoformat(),
    }
...
def generate_sdp_deployment_report(config: dict) -> dict:
    ...
    return {
        ...
        "summary": {
            ...
            "spa_enabled": all(
                g.get("spa_enabled") for g in config.get("gateways", [])
            ),
            "mtls_enabled": all(
                g.get("mtls_enabled") for g in config.get("gateways", [])
            ),
        },
        "recommendations": [
            f["finding"]
...
def main():
    ...
    parser.add_argument(
        "--scan", type=str, help="Gateway host to scan for invisibility"
    )
    parser.add_argument(
        "--scan-ports", type=str, default="1-1024", help="Port range to scan"
    )
    parser.add_argument(
        "--check-tls", type=str, help="Host:port to check TLS certificate"
    )
    ...
    if args.config:
        ...
        print(
            f"SDP Status: {report['deployment_status']} (Score: {report['security_score']})"
        )
