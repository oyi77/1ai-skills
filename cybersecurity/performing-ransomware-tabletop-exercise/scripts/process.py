...
THREAT_ACTORS = {
    "lockbit": {
        ...
        "ttps": [
            ...
            "Deletes shadow copies with vssadmin",
    ...
    "alphv": {
        "name": "ALPHV/BlackCat",
        "initial_access": [
            "Compromised credentials from IABs",
        ...
        "ttps": [
            "Written in Rust (cross-platform)",
            "Targets ESXi hypervisors",
            "Triple extortion (encrypt + leak + DDoS)",
            "Destroys backups before encryption",
        ],
    ...
    "clop": {
        "name": "Cl0p",
        "initial_access": [
            "Zero-day exploitation of file transfer platforms (MOVEit, GoAnywhere, Accellion)",
            "Supply chain compromise",
        ],
        "tools": ["FlawedAmmyy RAT", "SDBot", "TrueBot", "Custom exfiltration tools"],
        "ttps": [
            "Mass exploitation campaigns",
            "Data theft without encryption in many cases",
            "Targets managed file transfer (MFT) platforms",
            "Extended extortion timeline",
        ],
    ...
    "rhysida": {
        ...
        "ttps": [
            ...
            "Deletes VSS and disables Windows recovery",
            "Auctions stolen data on leak site",
        ],
...
INDUSTRY_PROFILES = {
    ...
    "education": {
        ...
        "data_types": [
            ...
            "Employee PII",
            "Healthcare records (student health)",
        ],
...
class TabletopGenerator:
    ...
    def __init__(self, org_name: str, industry: str, threat_actor: str):
        ...
        if industry not in INDUSTRY_PROFILES:
            raise ValueError(
                f"Unknown industry: {industry}. Choose from: {list(INDUSTRY_PROFILES.keys())}"
            )
        if threat_actor not in THREAT_ACTORS:
            raise ValueError(
                f"Unknown threat actor: {threat_actor}. Choose from: {list(THREAT_ACTORS.keys())}"
            )
    ...
    def generate_scenario(
        self,
        encrypted_percentage: int = 60,
        data_exfiltrated: bool = True,
        backups_intact: bool = True,
        ransom_amount: str = "$2,000,000",
    ) -> ExerciseScenario:
        ...
        backup_status = (
            "Immutable backup copies appear intact. Primary backups on the NAS are encrypted."
            if backups_intact
            else "All backup systems have been compromised. The attackers deleted backup catalogs before encrypting."
        )
        scenario.phases.append(
            {
                "number": 3,
    ...
    def export_scenario(self, output_dir: str) -> str:
        ...
        output_path = (
            Path(output_dir)
            / f"ttx_scenario_{self.org_name.replace(' ', '_')}_{datetime.now().strftime('%Y%m%d')}.json"
        )
...
class ExerciseEvaluator:
    ...
    def generate_aar(self) -> str:
        ...
        lines.append(
            f"Areas scoring below 3: {sum(1 for e in self.evaluations if e.score < 3)}"
        )
        lines.append("")
        lines.append("=" * 70)
...
def main():
    ...
    evaluator.add_evaluation(
        ...
        score=4,
        strengths=[
            "SOC correctly identified Cobalt Strike indicators",
            "Incident was declared within 30 minutes",
        ],
        gaps=["No documented criteria for incident declaration threshold"],
        remediation_actions=[
            "Document incident declaration criteria with specific trigger conditions"
        ],
    )
    evaluator.add_evaluation(
        ...
        gaps=[
            "No pre-established payment decision framework",
            "No pre-engaged negotiation firm",
            "No cryptocurrency procurement mechanism identified",
        ],
        remediation_actions=[
            "Establish ransom payment decision matrix with executive sign-off",
