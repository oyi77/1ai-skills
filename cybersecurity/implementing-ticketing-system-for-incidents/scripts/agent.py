...
class ServiceNowClient:
    ...
    def create_incident(
        self, short_desc, description, urgency=2, impact=2, category="Security"
    ):
        ...
        resp = self.session.post(
            f"{self.base_url}/table/incident", json=data, timeout=30
        )
    ...
    def update_incident(self, sys_id, update_data):
        ...
        resp = self.session.patch(
            f"{self.base_url}/table/incident/{sys_id}", json=update_data, timeout=30
        )
        resp.raise_for_status()
    ...
    def close_incident(self, sys_id, close_notes, close_code="Solved (Permanently)"):
        ...
        return self.update_incident(
            sys_id, {"state": "7", "close_code": close_code, "close_notes": close_notes}
        )
...
class TheHiveClient:
    ...
    def create_task(self, case_id, title, description="", group="default"):
        ...
        resp = self.session.post(
            f"{self.base_url}/api/case/{case_id}/task", json=data, timeout=30
        )
        resp.raise_for_status()
        return resp.json()
    ...
    def add_observable(self, case_id, data_type, data_value, tlp=2, message=""):
        ...
        resp = self.session.post(
            f"{self.base_url}/api/case/{case_id}/artifact", json=obs_data, timeout=30
        )
        resp.raise_for_status()
        return resp.json()
    ...
    def close_case(self, case_id, summary, impact_status="NoImpact"):
        ...
        resp = self.session.patch(
            f"{self.base_url}/api/case/{case_id}", json=data, timeout=30
        )
        resp.raise_for_status()
        return resp.json()
...
def run_ticketing_audit(snow_client=None, hive_client=None):
    ...
    if snow_client:
        ...
        for inc in incidents[:10]:
            print(
                f"  [{inc.get('priority', 'N/A')}] {inc.get('number')}: "
    ...
    if hive_client:
        ...
        for case in cases[:10]:
            print(
                f"  [Sev:{case.get('severity', 'N/A')}] #{case.get('caseId')}: "
                f"{case.get('title', '')[:50]}"
            )
...
def main():
    ...
    snow.add_argument(
        "--create", nargs=2, metavar=("TITLE", "DESC"), help="Create incident"
    )
    ...
    hive.add_argument(
        "--create", nargs=2, metavar=("TITLE", "DESC"), help="Create case"
    )
