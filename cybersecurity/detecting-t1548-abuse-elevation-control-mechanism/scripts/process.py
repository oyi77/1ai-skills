...
def detect_elevation_abuse(events: list[dict]) -> list[dict]:
    ...
    for event in events:
        ...
        if event_code in ("12", "13"):
            ...
            if (
                image_name in AUTO_ELEVATE_BINARIES
                and parent_name not in EXPECTED_PARENTS
            ):
                findings.append(
                    {
                        "timestamp": timestamp,
...
def run_hunt(input_path: str, output_dir: str) -> None:
    print(
        f"[*] T1548 Elevation Control Abuse Hunt - {datetime.datetime.now().isoformat()}"
    )
    ...
    with open(
        output_path / "elevation_abuse_findings.json", "w", encoding="utf-8"
    ) as f:
        json.dump(
            {
                "hunt_id": f"TH-UAC-{datetime.date.today().isoformat()}",
                "findings": findings,
            },
            f,
            indent=2,
...
def main():
    parser = argparse.ArgumentParser(
        description="T1548 Elevation Control Abuse Detection"
    )
