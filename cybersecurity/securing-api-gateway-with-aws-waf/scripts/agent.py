    ...
    for rt in resource_types:
        try:
            resp = client.list_resources_for_web_acl(
                WebACLArn=web_acl_arn, ResourceType=rt
            )
...
def get_sampled_requests(
    client, web_acl_arn, rule_metric, scope="REGIONAL", minutes=60
):
...
def audit_web_acl(client, name, acl_id, scope="REGIONAL"):
    ...
    for rec in recommended:
        if not any(rec in rn for rn in rule_names):
            findings.append(
                f"MISSING: Recommended managed rule group '{rec}' not found"
            )
    has_rate_rule = any(
        "RateBasedStatement" in json.dumps(r.get("Statement", {})) for r in rules
    )
...
def main():
    ...
    parser.add_argument(
        "action",
        choices=["list", "details", "audit", "metrics", "samples", "rate-keys"],
    )
    ...
    parser.add_argument(
        "--hours", type=int, default=24, help="Lookback hours for metrics"
    )
    ...
    if args.action == "list":
        ...
        get_rate_based_keys(
            client, args.name, args.id, args.rule_name or "RateLimitPerIP"
        )
