...
def parse_auth_log(path: str, output_dir: str) -> str:
    ...
    with open(path, "r", errors="replace") as f:
        for line in f:
            ...
            if m:
                failed.append(
                    {"time": m.group(1), "user": m.group(2), "ip": m.group(3)}
                )
                continue
    ...
    report = {
        ...
        "top_successful": successful[:50],
        "top_failed": failed[:50],
        "top_sudo": sudo_cmds[:50],
    }
    out = os.path.join(output_dir, "linux_auth_forensics.json")
    with open(out, "w") as f:
        json.dump(report, f, indent=2)
    print(
        f"[*] OK:{len(successful)} FAIL:{len(failed)} SUDO:{len(sudo_cmds)} BruteForce:{len(brute_force)}"
    )
    return out
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: process.py <auth.log> <output_dir>")
        sys.exit(1)
