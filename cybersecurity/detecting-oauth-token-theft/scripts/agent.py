...
def detect_impossible_travel(sign_ins, max_speed_kmh=900):
    ...
    for user, events in by_user.items():
        ...
        for i in range(1, len(sorted_events)):
            ...
            if not all(k in prev for k in ("lat", "lon")) or not all(
                k in curr for k in ("lat", "lon")
            ):
                continue
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=900,
        help="Max travel speed km/h (default: 900)",
    )
    ...
    for a in report["alerts"]:
        print(
            f"  [{a['severity']}] {a['type']}: {a.get('user', '')} - {a.get('distance_km', a.get('ips', a.get('device', '')))}"
        )
    ...
    print(
        json.dumps(
            {"total_alerts": len(report["alerts"]), "by_type": dict(by_type)}, indent=2
        )
    )
