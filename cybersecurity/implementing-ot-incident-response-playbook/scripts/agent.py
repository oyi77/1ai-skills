...
def assess_incident(incident_type, affected_assets):
    ...
    has_safety_system = any(
        a.get("type") in ("SIS", "safety_plc", "emergency_shutdown")
    ...
    return {
        ...
        "escalated_severity": (
            "critical" if has_safety_system else incident_info["severity"]
        ),
...
def main():
    ...
    parser.add_argument(
        "--incident-type", required=True, choices=list(OT_INCIDENT_TYPES.keys())
    )
