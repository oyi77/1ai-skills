...
def main():
    ...
    parser.add_argument(
        "--keyword", required=True, help="Keyword for bucket name permutation"
    )
    parser.add_argument("--project", help="GCP project ID for IAM audit")
    parser.add_argument("--output", default="gcp_pentest_report.json")
    parser.add_argument(
        "--skip-buckets", action="store_true", help="Skip bucket enumeration"
    )
    ...
    report = generate_report(
        buckets, iam_findings, sa_findings, esc_paths, args.keyword, args.project
    )
