    ...
    def detect_sod_violations(self) -> List[Dict]:
        ...
        for user_id, ents in user_entitlements.items():
            for rule in self.sod_rules:
                has_a = any(
                    e.application == rule.application_a
                    and e.entitlement == rule.entitlement_a
                    for e in ents
                )
                has_b = any(
                    e.application == rule.application_b
                    and e.entitlement == rule.entitlement_b
                    for e in ents
                )
                if has_a and has_b:
    ...
    def generate_compliance_report(self) -> str:
        ...
        if self.sod_violations:
            ...
            for v in self.sod_violations:
                lines.append(
                    f"  [{v['severity'].upper()}] {v['user_name']} ({v['user_id']})"
                )
                lines.append(f"    Rule: {v['description']}")
                lines.append(
                    f"    Conflict: {v['entitlement_a']} <-> {v['entitlement_b']}"
                )
            lines.append("")
        ...
        for reviewer, progress in metrics["reviewer_progress"].items():
            ...
            lines.append(
                f"  {reviewer}: {progress['completed']}/{progress['total']} ({pct}%)"
            )
        lines.append("")
        ...
        if revoked:
            ...
            for e in revoked:
                lines.append(
                    f"  {e.user_name} - {e.application}:{e.entitlement} [{e.risk_level}]"
                )
            lines.append("")
...
def main():
    ...
    engine.process_decision(
        ...
        "AdminAccess",
        "revoked",
        "Stale access - user no longer needs admin",
    )
    engine.process_decision(
