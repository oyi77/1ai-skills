...
def scan_bootkit_signatures(data):
    ...
    for sig, name in BOOTKIT_SIGNATURES.items():
        if sig in data:
            ...
            matches.append(
                {"signature": name, "offset": offset, "clean": "clean" in name}
            )
    return matches
...
def analyze_boot_code(mbr_data):
    ...
    if b"\xf3\xa4" in boot_code or b"\xf3\xa5" in boot_code:
        suspicious_patterns.append(
            "REP MOVSB/MOVSW (memory copy, possible code relocation)"
        )
    return {
        "entropy": entropy,
        "sha256": sha256,
...
if __name__ == "__main__":
    ...
    if os.path.exists(demo_mbr):
        ...
        print(
            f"[*] MBR Signature: 0x{sig_hex.upper()} ({'Valid' if valid else 'INVALID'})"
        )
