    ...
    for field_name, injected_value in MASS_ASSIGNMENT_PAYLOADS:
        ...
        try:
            ...
            if method == "PUT":
                ...
                resp = requests.patch(
                    url, json={field_name: injected_value}, headers=h, timeout=10
                )
            else:
            ...
            if resp.status_code in (200, 201, 204):
                ...
                if (
                    ...
                    and verify.json().get(field_name) == injected_value
                ):
                    sev = (
                        "CRITICAL"
                        if field_name in ("role", "is_admin", "permissions")
                        else "HIGH"
                    )
                    findings.append(
                        {
                    ...
                    if field_name in original_data:
                        requests.patch(
                            url,
                            json={field_name: original_data[field_name]},
                            headers=h,
                            timeout=10,
                        )
...
def main():
    parser = argparse.ArgumentParser(
        description="BOPLA Vulnerability Scanner (OWASP API3:2023)"
    )
    parser.add_argument("--base-url", required=True, help="API base URL")
    parser.add_argument(
        "--endpoint", required=True, help="Endpoint to test (e.g. /api/v1/users/1)"
    )
    parser.add_argument("--token", help="Bearer token for Authorization header")
    parser.add_argument(
        "--expected-fields", nargs="+", default=[], help="Expected response fields"
    )
    parser.add_argument(
        "--test", choices=["exposure", "mass_assignment", "both"], default="both"
    )
    ...
    if args.test in ("exposure", "both"):
        results["findings"].extend(
            test_excessive_exposure(
                args.base_url, args.endpoint, args.expected_fields, headers
            )
        )
