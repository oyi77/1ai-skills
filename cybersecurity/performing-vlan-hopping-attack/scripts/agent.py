...
def create_vlan_interface(iface, vlan_id, ip_addr):
    ...
    subprocess.run(
        ["ip", "link", "set", vlan_iface, "up"], capture_output=True, timeout=120
    )
...
def run_assessment(iface="eth0", target_vlans=None, target_ips=None):
    ...
    for vlan_id in target_vlans:
        ...
        report["tests"].append(
            {"test": f"Double Tagging VLAN {vlan_id}", "result": dt_result}
        )
...
def print_report(report):
    ...
    print(
        f"Interface: {report['interface']['interface']} ({report['interface']['mac']})"
    )
