...
def inspect_container(container_id):
    ...
    try:
        result = subprocess.check_output(
            ["docker", "inspect", container_id], text=True, errors="replace", timeout=10
        )
...
def audit_container(container_id):
    ...
    if host_config.get("Privileged"):
        findings.append(
            {
                "check": "CIS 5.4",
                "issue": "Container runs privileged",
                "severity": "CRITICAL",
            }
        )
    if host_config.get("NetworkMode") == "host":
        findings.append(
            {"check": "CIS 5.10", "issue": "Uses host network", "severity": "HIGH"}
        )
    if host_config.get("PidMode") == "host":
        findings.append(
            {
                "check": "CIS 5.15",
                "issue": "Shares host PID namespace",
                "severity": "HIGH",
            }
        )
    if host_config.get("IpcMode") == "host":
        findings.append(
            {
                "check": "CIS 5.16",
                "issue": "Shares host IPC namespace",
                "severity": "HIGH",
            }
        )
    ...
    if not user or user == "root" or user == "0":
        findings.append(
            {"check": "CIS 4.1", "issue": "Container runs as root", "severity": "HIGH"}
        )
    ...
    if "SYS_ADMIN" in cap_add:
        findings.append(
            {
                "check": "CIS 5.3",
                "issue": "SYS_ADMIN capability added",
                "severity": "CRITICAL",
            }
        )
    if "NET_ADMIN" in cap_add:
        findings.append(
            {
                "check": "CIS 5.3",
                "issue": "NET_ADMIN capability added",
                "severity": "HIGH",
            }
        )
    ...
    if memory == 0:
        findings.append(
            {"check": "CIS 5.14", "issue": "No memory limit set", "severity": "MEDIUM"}
        )

    cpu_shares = host_config.get("CpuShares", 0)
    if cpu_shares == 0:
        findings.append(
            {"check": "CIS 5.13", "issue": "No CPU limit set", "severity": "LOW"}
        )
    ...
    if restart == "always":
        findings.append(
            {
                "check": "CIS 5.14",
                "issue": "RestartPolicy=always (use on-failure)",
                "severity": "LOW",
            }
        )
    ...
    for mount in mounts:
        ...
        if src in sensitive:
            findings.append(
                {
                    "check": "CIS 5.5",
                    "issue": f"Sensitive host mount: {src}",
                    "severity": "HIGH",
                }
            )
    ...
    if args.all:
        ...
        for c in containers:
            ...
            report["audits"].append(
                {"container": c.get("Names", cid), "findings": findings}
            )
    elif args.container:
