...
class TIPEvaluator:
    ...
    EVALUATION_CRITERIA = {
        "core_functions": {
            "stix_support": {
                "weight": 10,
                "description": "STIX 2.1 import/export support",
            },
            "taxii_server": {"weight": 8, "description": "TAXII 2.1 server capability"},
            "rest_api": {"weight": 9, "description": "RESTful API for automation"},
            "deduplication": {
                "weight": 7,
                "description": "Indicator deduplication and TTL management",
            },
            "tlp_enforcement": {
                "weight": 8,
                "description": "TLP classification enforcement",
            },
            "attack_mapping": {"weight": 6, "description": "MITRE ATT&CK integration"},
            "graph_viz": {
                "weight": 5,
                "description": "Graph visualization of relationships",
            },
        },
        "integrations": {
            "siem_integration": {
                "weight": 9,
                "description": "SIEM bi-directional integration",
            },
            "edr_integration": {"weight": 7, "description": "EDR IOC push capability"},
            "soar_integration": {
                "weight": 7,
                "description": "SOAR playbook integration",
            },
            "firewall_integration": {
                "weight": 6,
                "description": "Firewall blocklist export",
            },
            "feed_ingestion": {
                "weight": 8,
                "description": "Multiple feed source support",
            },
        },
        "operations": {
            "analyst_workflow": {
                "weight": 7,
                "description": "Investigation workflow tools",
            },
...
if __name__ == "__main__":
    ...
    if action == "compare":
        ...
        token = os.environ.get(
            "OPENCTI_TOKEN", sys.argv[3] if len(sys.argv) > 3 else ""
        )
        if url and token:
        ...
        print(
            "Usage: agent.py [compare|template|test-misp [url key]|test-opencti [url token]]"
        )
