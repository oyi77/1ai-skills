...
class CalicoNetworkPolicyAgent:
    ...
    def list_calico_policies(self):
        ...
        policies = []
        stdout, rc = self._run_calicoctl(
            ["get", "networkpolicy", "-o", "json", "--all-namespaces"]
        )
    ...
    def find_unprotected_namespaces(self):
        ...
        unprotected = [
            ns for ns in namespaces if ns not in protected and ns not in system_ns
        ]
    ...
    def generate_report(self):
        ...
        report = {
            ...
            "k8s_network_policies": {
                "count": len(k8s_policies),
                "policies": k8s_policies,
            },
            "calico_policies": {
                "count": len(calico_policies),
                "policies": calico_policies,
            },
...
def main():
    ...
    parser.add_argument(
        "--output-dir",
        default="./calico_policy_audit",
        help="Output directory for report",
    )
    args = parser.parse_args()

    os.makedirs(args.output_dir, exist_ok=True)
    agent = CalicoNetworkPolicyAgent(
        kubeconfig=args.kubeconfig, output_dir=args.output_dir
    )
