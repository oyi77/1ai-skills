...
def get_guardduty_crypto_findings():
    ...
    result = aws_cli(
        [
            ...
            detector_id,
            "--finding-criteria",
            json.dumps({"Criterion": {"type": {"Eq": crypto_types}}}),
        ]
    )
        ...
        events.append(
            {
                ...
                "instance_type": detail.get("requestParameters", {}).get(
                    "instanceType"
                ),
            }
        )
...
def isolate_mining_instance(instance_id):
    ...
    sg_result = aws_cli(
        [
            ...
            "--group-name",
            f"isolation-{instance_id}",
            "--description",
            "Isolation SG for mining instance",
        ]
    )
...
if __name__ == "__main__":
    ...
    if action == "report":
        ...
        print(
            json.dumps(check_cloudtrail_instance_launches(hours), indent=2, default=str)
        )
        ...
        print(
            "Usage: agent.py [report|findings|launches [hours]|flow-logs [log-group]|isolate <instance-id>]"
        )
