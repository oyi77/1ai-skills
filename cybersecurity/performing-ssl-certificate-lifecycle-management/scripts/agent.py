...
class CertLifecycleAgent:
    ...
    def generate_csr(
        self, common_name, org="", country="US", san_names=None, key_type="ecdsa"
    ):
    ...
    def fetch_remote_cert(self, hostname, port=443):
        ...
        try:
            ...
            not_after = datetime.strptime(pem_info["notAfter"], "%b %d %H:%M:%S %Y %Z")
    ...
    def check_expiring(self, threshold_days=30):
        ...
        return [
            ...
            if c.get("days_remaining", 999) <= threshold_days and "error" not in c
        ]
...
def main():
    hosts = (
        sys.argv[1:]
        if len(sys.argv) > 1
        else ["google.com", "github.com", "expired.badssl.com"]
    )
