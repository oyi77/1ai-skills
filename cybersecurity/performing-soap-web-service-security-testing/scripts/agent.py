...
class SOAPSecurityTester:
    ...
    def _send_soap(self, body_xml, soap_action="", timeout=10):
        ...
        return requests.post(
            self.endpoint_url, data=body_xml, headers=headers, timeout=timeout
        )
...
def main():
    wsdl = (
        sys.argv[1]
        if len(sys.argv) > 1
        else os.environ.get("SOAP_WSDL_URL", "http://localhost:8080/ws?wsdl")
    )
