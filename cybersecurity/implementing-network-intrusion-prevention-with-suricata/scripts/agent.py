                        ...
                        alerts.append(
                            {
                                ...
                                "signature_id": event.get("alert", {}).get(
                                    "signature_id", 0
                                ),
...
def analyze_alert_distribution(alerts):
    ...
    return {
        "top_signatures": dict(
            sorted(by_sig.items(), key=lambda x: x[1], reverse=True)[:15]
        ),
        ...
        "top_source_ips": dict(
            sorted(by_src.items(), key=lambda x: x[1], reverse=True)[:10]
        ),
    }
            ...
            patterns.append(
                {
                    ...
                    "pattern": (
                        "multi_stage_attack" if len(sigs) > 3 else "repeated_exploit"
                    ),
                }
            )
...
def check_suricata_status():
    ...
    rule_count_cmd = subprocess.run(
        ["suricata", "--build-info"], capture_output=True, text=True, timeout=120
    )
...
def main():
    ...
    parser.add_argument(
        "--limit", type=int, default=50000, help="Max log lines to parse"
    )
    parser.add_argument(
        "--noisy-threshold",
        type=int,
        default=500,
        help="Alert count threshold for noisy rules",
    )
