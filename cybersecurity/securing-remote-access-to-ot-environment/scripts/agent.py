...
class OTRemoteAccessAgent:
    ...
    def request_session(
        self, user_id, role, source_ip, target, target_ip, protocol, purpose
    ):
    ...
    def activate_session(self, sid, mfa_verified=True):
        ...
        s["recording_path"] = (
            f"/recordings/{sid}_{datetime.utcnow().strftime('%Y%m%d')}.mp4"
        )
        self._log(
            "ACTIVATED",
            s["user_id"],
            s["target"],
            f"MFA={'OK' if mfa_verified else 'FAIL'}",
        )
        return True, "Active"
        ...
        for sid, s in self.sessions.items():
            if s["state"] == SessionState.ACTIVE and not s["mfa_verified"]:
                issues.append(
                    {
                        "session": sid,
                        "issue": "Active session without MFA (CIP-005-7 R2.4)",
                    }
                )
            if s["role"] == UserRole.VENDOR:
                ...
                if policy.get("requires_co_attendance") and "co_attendant" not in s:
                    issues.append(
                        {
                            "session": sid,
                            "issue": "Vendor session without co-attendance",
                        }
                    )
        return issues
