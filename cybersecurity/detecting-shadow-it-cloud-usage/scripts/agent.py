...
def extract_domain(url_or_host):
    ...
    if tldextract:
        ...
        return (
            f"{ext.domain}.{ext.suffix}".lower() if ext.suffix else url_or_host.lower()
        )
    ...
    with open(filepath, "r") as f:
        ...
        for row in reader:
            domain = extract_domain(
                row.get("dst_domain", row.get("domain", row.get("url", "")))
            )
            records.append(
                {
...
def full_audit(log_path, log_type="proxy", approved_list=None):
    ...
    unauthorized = [
        a for a in analysis if not a["approved"] and a["category"] != "unknown"
    ]
    return {
...
def main():
    parser = argparse.ArgumentParser(
        description="Shadow IT Cloud Usage Detection Agent"
    )
    parser.add_argument("log_file", help="Path to log file")
    parser.add_argument(
        "--type",
        choices=["proxy", "dns", "csv"],
        default="proxy",
        help="Log file format",
    )
    parser.add_argument(
        "--approved", help="Path to approved domains list (one per line)"
    )
