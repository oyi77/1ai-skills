...
def scan_crontabs():
    ...
    user_crontabs = subprocess.run(
        [
            "bash",
            "-c",
            'for u in $(cut -d: -f1 /etc/passwd); do crontab -l -u $u 2>/dev/null && echo "__USER:$u"; done',
        ],
        capture_output=True,
        text=True,
        timeout=120,
...
def _scan_cron_file(filepath):
    ...
    try:
        ...
        entries.append(
            {"type": "cron_file", "path": filepath, "error": "Permission denied"}
        )
    return entries
...
def _assess_cron_risk(command):
    ...
    for pattern in SUSPICIOUS_PATTERNS:
        if re.search(pattern, command, re.IGNORECASE):
            return "critical"
    if any(
        kw in command.lower() for kw in ["wget", "curl", "/tmp/", "base64", "chmod"]
    ):
        return "high"
    return "low"
...
def scan_systemd_units():
    ...
    for base_path in SYSTEMD_PATHS:
        if not os.path.isdir(base_path):
            continue
        for unit_file in glob.glob(os.path.join(base_path, "*.service")) + glob.glob(
            os.path.join(base_path, "*.timer")
        ):
            try:
                ...
                for ex in exec_lines:
                    if any(
                        s in ex
                        for s in ["/tmp/", "/dev/shm/", "curl", "wget", "bash -c"]
                    ):
                        risk = "high"
                    for pattern in SUSPICIOUS_PATTERNS:
...
def scan_shell_profiles():
    ...
    for filepath in etc_profiles:
        dpkg = subprocess.run(
            ["dpkg", "-S", filepath], capture_output=True, text=True, timeout=120
        )
...
def main():
    ...
    scans = (
        set(args.scan)
        if "all" not in args.scan
        else {"cron", "systemd", "ldpreload", "profiles", "ssh"}
    )
    ...
    print(
        f"[+] Risk: critical={report['risk_summary'].get('critical', 0)} "
