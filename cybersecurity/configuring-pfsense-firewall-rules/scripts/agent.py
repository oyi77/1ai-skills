...
class PfSenseAPI:
    ...
    def post(self, endpoint, data):
        resp = self.session.post(
            f"{self.base_url}/api/v1/{endpoint}", json=data, timeout=30
        )
        resp.raise_for_status()
        return resp.json()
...
def configure_nat_port_forward(
    ...
    logger.info(
        "Created NAT: %s:%s -> %s:%s", interface, external_port, target_ip, target_port
    )
    return result
...
def generate_report(rules, audit_findings, nat_rules=None):
    ...
    print(
        f"PFSENSE FIREWALL REPORT: {len(rules)} rules, {len(audit_findings)} findings"
    )
    return report
...
def main():
    ...
    parser.add_argument(
        "--url", required=True, help="pfSense base URL (https://192.168.1.1)"
    )
    ...
    parser.add_argument(
        "--audit-only", action="store_true", help="Audit rules without changes"
    )
    parser.add_argument("--setup-dmz", action="store_true", help="Create DMZ rules")
    parser.add_argument(
        "--setup-guest", action="store_true", help="Create guest isolation rules"
    )
