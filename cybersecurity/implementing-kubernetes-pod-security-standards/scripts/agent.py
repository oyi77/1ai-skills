...
PSS_LEVELS = {
    ...
    "baseline": {
        "order": 1,
        "description": "Minimally restrictive, prevents known escalations",
    },
    "restricted": {
        "order": 2,
        "description": "Heavily restricted, hardened best practices",
    },
}
...
def main():
    parser = argparse.ArgumentParser(
        description="Kubernetes Pod Security Standards Agent"
    )
    ...
    parser.add_argument(
        "--action",
        choices=["audit-ns", "audit-pods", "generate", "full"],
        default="full",
    )
    ...
    if args.action in ("audit-ns", "full"):
        ...
        if isinstance(ns_audit, list):
            unprotected = sum(
                1 for n in ns_audit if not n["protected"] and not n["system"]
            )
