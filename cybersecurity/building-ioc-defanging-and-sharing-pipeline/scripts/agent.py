    ...
    for ioc_type, pattern in IOC_PATTERNS.items():
        ...
        if ioc_type == "domain":
            matches = {
                m
                for m in matches
                if m.split(".")[-1] not in {"py", "js", "json", "xml", "yml"}
                and m not in EXCLUSION_DOMAINS
                and not any(excl in m for excl in EXCLUSION_DOMAINS)
            }
...
def to_stix_bundle(iocs):
    ...
    bundle = {
        "type": "bundle",
        "id": f"bundle--{hash(str(iocs)) % (10**12):012d}",
        "objects": objects,
    }
...
def defang_all(iocs):
    ...
    for ioc_type, values in iocs.items():
        defanged[ioc_type] = [
            {"original": v, "defanged": defang_ioc(v, ioc_type)} for v in values
        ]
    return defanged
...
def generate_report(iocs, defanged, stix_bundle):
    ...
    print(
        f"IOC PIPELINE REPORT: {total} IOCs extracted, {len(stix_bundle.get('objects', []))} STIX indicators"
    )
    return report
...
def main():
    ...
    parser.add_argument(
        "--enrich", action="store_true", help="Enrich IOCs with threat intel"
    )
    parser.add_argument(
        "--vt-key", default=os.environ.get("VT_API_KEY", ""), help="VirusTotal API key"
    )
    parser.add_argument(
        "--abuseipdb-key",
    ...
    if args.enrich:
        ...
        for ioc_type, values in iocs.items():
            for value in values[:10]:
                enrichments.append(
                    enrich_ioc(value, ioc_type, args.vt_key, args.abuseipdb_key)
                )
