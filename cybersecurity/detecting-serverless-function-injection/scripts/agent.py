...
INJECTION_PATTERNS = {
    "python": [
        ...
        {
            "pattern": r"\bsubprocess\.call\s*\(.*shell\s*=\s*True",
        ...
        {
            "pattern": r"\bsubprocess\.Popen\s*\(.*shell\s*=\s*True",
        ...
        {
            "pattern": r"\bjinja2\.Template\s*\(.*event",
            "sink": "jinja2.Template() with event data",
            "severity": "high",
            "cwe": "CWE-1336",
        },
        {
        ...
        {
            "pattern": r"f['\"].*\{.*event.*\}.*['\"].*\.execute\(",
            "sink": "SQL via f-string with event data",
            "severity": "critical",
            "cwe": "CWE-89",
        },
        {
            "pattern": r"['\"].*%s.*['\"].*%.*event",
            "sink": "SQL via string formatting with event data",
            "severity": "critical",
            "cwe": "CWE-89",
        },
    ],
    "nodejs": [
        ...
        {
            "pattern": r"\bchild_process\.execSync\s*\(",
        ...
        },
        {
            ...
            "sink": "vm.runInNewContext()",
            "severity": "critical",
            "cwe": "CWE-95",
        },
        {
            "pattern": r"\bvm\.runInThisContext\s*\(",
            "sink": "vm.runInThisContext()",
            "severity": "critical",
            "cwe": "CWE-95",
        },
        {
            "pattern": r"\brequire\s*\(\s*['\"]child_process['\"]\s*\)",
            "sink": "require('child_process')",
            "severity": "medium",
            "cwe": "CWE-78",
        },
        {
            "pattern": r"`.*\$\{.*event.*\}`.*exec",
            "sink": "Template literal command injection",
            "severity": "critical",
            "cwe": "CWE-78",
        },
    ],
    ...
    try:
        ...
        for root, dirs, files in os.walk(extract_dir):
            ...
            dirs[:] = [
                d
                for d in dirs
                if d not in ("node_modules", "vendor", "__pycache__", ".git")
            ]
                            ...
                            findings.append(
                                {
                                    ...
                                    "confidence": (
                                        "high" if event_data_involved else "medium"
                                    ),
                                }
                            )
        ...
        if role_name in checked_roles:
            ...
            try:
                ...
                for policy in attached["AttachedPolicies"]:
                    ...
                    try:
                        policy_info = iam_client.get_policy(
                        ...
                        for stmt in policy_doc["PolicyVersion"]["Document"].get(
                            "Statement", []
                        ):
                            ...
                            for action in actions:
                                if action in dangerous_actions or action == "*":
                                    role_findings["dangerous_permissions"].append(
                                        action
                                    )
                    except ClientError:
                        continue
    ...
    for event_name in suspicious_events:
        try:
            ...
            for event in response.get("Events", []):
                ...
                if (
                    "console.amazonaws.com" not in user_agent
                    and "cloudformation" not in user_agent.lower()
                ):
                    if "UpdateFunctionCode" in event_name:
                        finding["suspicious"] = True
                        finding["indicators"].append(
                            "Function code updated outside console/CloudFormation"
                        )
                ...
                if "role" in req_params and "UpdateFunctionConfiguration" in event_name:
                    finding["suspicious"] = True
                    finding["indicators"].append(
                        f"Execution role changed to: {req_params['role']}"
                    )

                # Check for layer additions
                if (
                    "layers" in req_params
                    and "UpdateFunctionConfiguration" in event_name
                ):
                    finding["suspicious"] = True
                    finding["indicators"].append(
                        f"Layers modified: {req_params['layers']}"
                    )
                ...
                if event_hour < 6 or event_hour > 22:
                    finding["indicators"].append(
                        f"Modification at unusual hour: {event_hour}:00 UTC"
                    )
...
def generate_report(
    ...
    report = {
        ...
        "summary": {
            ...
            "suspicious_modifications": len(
                [f for f in cloudtrail_findings if f.get("suspicious")]
            ),
        },
...
def main():
    parser = argparse.ArgumentParser(
        description="Serverless Function Injection Detection Agent"
    )
    parser.add_argument("--region", default="us-east-1", help="AWS region")
    parser.add_argument(
        "--functions", nargs="+", help="Specific function names to scan (default: all)"
    )
    parser.add_argument(
        "--scan-code",
        action="store_true",
        help="Download and scan function code for injection sinks",
    )
    parser.add_argument(
        "--cloudtrail-days",
        type=int,
    ...
    parser.add_argument(
        "--output",
        default="serverless_injection_report.json",
    ...
    if args.functions:
        all_functions = [
            f for f in all_functions if f["function_name"] in args.functions
        ]
        ...
        try:
            for func in all_functions:
                if func["runtime_family"] in INJECTION_PATTERNS:
                    logger.info(
                        "Scanning %s (%s)", func["function_name"], func["runtime"]
                    )
    ...
    cloudtrail_findings = check_cloudtrail_for_modifications(
