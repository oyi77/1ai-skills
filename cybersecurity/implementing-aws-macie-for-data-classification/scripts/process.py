...
def create_classification_job(session, bucket_names, account_id, job_name=None):
    ...
    response = client.create_classification_job(
        ...
        name=job_name,
        s3JobDefinition={
            "bucketDefinitions": [{"accountId": account_id, "buckets": bucket_names}]
        },
...
def get_bucket_statistics(session):
    ...
    response = client.describe_buckets(criteria={}, maxResults=50)

    buckets = response.get("buckets", [])
    print(f"\n[+] Monitoring {len(buckets)} S3 buckets:")
    print(
        f"{'Bucket':<40} {'Encryption':<15} {'Public':<10} {'Shared':<10} {'Objects':<10}"
    )
    print("-" * 85)
    ...
    for bucket in buckets:
        ...
        public_access = bucket.get("publicAccess", {}).get(
        ...
        print(
            f"{name:<40} {encryption:<15} {public_flag:<10} {shared_flag:<10} {obj_count:<10}"
        )
