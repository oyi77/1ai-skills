...
def get_guardduty_s3_findings():
    ...
    criteria = {
        "Criterion": {"type": {"Eq": s3_types}, "service.archived": {"Eq": ["false"]}}
    }
    result = aws_cli(
        [
            "guardduty",
...
def query_cloudtrail_s3_gets(bucket_name, hours=24):
    ...
    result = aws_cli(
        [
            ...
            "lookup-events",
            "--lookup-attributes",
            json.dumps([{"AttributeKey": "EventName", "AttributeValue": "GetObject"}]),
            "--start-time",
...
def check_s3_access_logging(bucket_name):
    ...
    return {
        ...
        "target_bucket": (
            result.get("LoggingEnabled", {}).get("TargetBucket") if enabled else None
        ),
    }
        ...
        "Statement": [
            {
                ...
                "Condition": {
                    "StringNotEquals": {
                        "aws:sourceVpce": "vpce-REPLACE_WITH_ENDPOINT_ID"
                    }
                },
            }
        ],
...
if __name__ == "__main__":
    ...
    if action == "report":
        ...
        print(
            json.dumps(
                query_cloudtrail_s3_gets(sys.argv[2], hours), indent=2, default=str
            )
        )
    elif action == "bulk" and len(sys.argv) > 2:
        print(
            json.dumps(
                detect_bulk_download_patterns(sys.argv[2]), indent=2, default=str
            )
        )
        ...
        print(
            "Usage: agent.py [report [bucket]|findings|gets <bucket> [hours]|bulk <bucket>|policy <bucket>]"
        )
