...
def run_snyk_test(
    project_path: str, severity_threshold: str = "low", extra_args: list = None
) -> dict:
...
def apply_quality_gate(
    findings: list[dict], max_critical: int = 0, max_high: int = 5
) -> dict:
    ...
    return {
        ...
        "reason": (
            "PASS"
            if passed
            else f"FAIL: {counts['critical']} critical (max {max_critical}), {counts['high']} high (max {max_high})"
        ),
    }
        ...
        results.append(
            {
                ...
                "level": (
                    "error" if f["severity"] in ("critical", "high") else "warning"
                ),
            }
        )
...
def generate_report(
    project_path: str, severity_threshold: str, max_critical: int, max_high: int
) -> dict:
...
def main():
    parser = argparse.ArgumentParser(
        description="SCA Dependency Scanning with Snyk Agent"
    )
    parser.add_argument("--project", required=True, help="Project directory to scan")
    parser.add_argument(
        "--severity", default="low", choices=["low", "medium", "high", "critical"]
    )
    parser.add_argument(
        "--max-critical",
        type=int,
        default=0,
        help="Max critical vulns for quality gate",
    )
    parser.add_argument(
        "--max-high", type=int, default=5, help="Max high vulns for quality gate"
    )
