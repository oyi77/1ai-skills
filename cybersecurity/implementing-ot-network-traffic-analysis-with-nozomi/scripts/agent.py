...
def detect_anomalies(alerts):
    ...
    return {
        cat: {"count": len(items), "samples": items[:3]}
        for cat, items in anomaly_types.items()
    }
...
def audit_asset_inventory(assets):
    ...
    return {
        ...
        "by_vendor": dict(
            sorted(by_vendor.items(), key=lambda x: x[1], reverse=True)[:10]
        ),
    }
...
def main():
    parser = argparse.ArgumentParser(
        description="Nozomi Networks OT Traffic Analysis Agent"
    )
    ...
    logger.info(
        "Nozomi: %d alerts, %d sessions, %d assets",
        len(alerts),
        len(sessions),
        len(assets),
    )
