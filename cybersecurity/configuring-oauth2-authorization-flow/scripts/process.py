...
class OAuth2Auditor:
    ...
    def _audit_grant_types(self):
        ...
        if not any(
            g in self.DEPRECATED_GRANTS for g in self.config.grant_types_supported
        ):
            self.findings.append(
                AuditFinding(
                    category="Grant Types",
    ...
    def _audit_redirect_uris(self):
        ...
        for uri in self.config.redirect_uris:
            ...
            elif uri.startswith("http://localhost") or uri.startswith(
                "http://127.0.0.1"
            ):
                self.findings.append(
                    AuditFinding(
    ...
    def _audit_scopes(self):
        ...
        if self.config.scopes_supported:
            granular_pattern = any(
                ":" in s or "." in s for s in self.config.scopes_supported
            )
    ...
    def _audit_token_endpoint_auth(self):
        ...
        if (
            "client_secret_post" in self.config.token_endpoint_auth_methods
            and "client_secret_basic" in self.config.token_endpoint_auth_methods
        ):
            self.findings.append(
                AuditFinding(
        ...
        has_secure = any(
            m in self.SECURE_AUTH_METHODS
            for m in self.config.token_endpoint_auth_methods
        )
        if has_secure:
            self.findings.append(
                AuditFinding(
    ...
    def _audit_discovery_endpoint(self):
        ...
        try:
            req = urllib.request.Request(
                discovery_url, headers={"User-Agent": "OAuth2-Auditor/1.0"}
            )
    ...
    def generate_report(self) -> str:
        ...
        lines.append(
            f"  Critical: {critical} | High: {high} | Medium: {len(by_severity.get('medium', []))} | Low: {len(by_severity.get('low', []))} | Info: {len(by_severity.get('info', []))}"
        )
        lines.append("")
        ...
        overall = (
            "FAIL" if critical > 0 else "NEEDS IMPROVEMENT" if high > 0 else "PASS"
        )
        lines.append("=" * 70)
