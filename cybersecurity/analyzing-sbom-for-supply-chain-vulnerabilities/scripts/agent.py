...
def parse_cyclonedx(sbom_data):
    ...
    for comp in sbom_data.get("components", []):
        ...
        for prop in comp.get("properties", []):
            if (
                prop.get("name") == "syft:cpe23"
                or "cpe" in prop.get("name", "").lower()
            ):
                cpe = prop.get("value")
                break
...
def query_nvd_by_keyword(keyword, api_key=None):
    ...
    try:
        ...
        print(
            f"    [WARN] NVD keyword search error for '{keyword}': {e}", file=sys.stderr
        )
        return []
...
def correlate_cves(components, api_key=None, skip_nvd=False):
    ...
    if not api_key:
        ...
        print(
            f"  Get a free key at: https://nvd.nist.gov/developers/request-an-api-key"
        )
    ...
    for idx, comp in enumerate(components):
        ...
        status = (
            f" {cve_count} CVEs (max CVSS: {comp.max_cvss})" if cve_count else " clean"
        )
        print(status)
...
def build_dependency_graph(components, dependencies):
    ...
    if not HAS_NETWORKX:
        print(
            "[WARN] networkx not installed. Dependency graph analysis skipped.",
            file=sys.stderr,
        )
        return None
...
def analyze_dependency_graph(G):
    ...
    if len(G.nodes) > 1:
        ...
        analysis["bottleneck_components"] = [
            {"ref": node, "centrality": round(cent, 4)}
...
def generate_report(
    ...
    report_lines.append(
        f"Dependencies:       {len(dependencies)} direct, ~{transitive} transitive"
    )
    report_lines.append("")
    ...
    report_lines.append(
        f"  {'TOTAL':10s}: {len(vuln_components):3d} components / {total_cves:3d} CVEs"
    )
    report_lines.append("")
    ...
    if critical_high:
        ...
        for i, comp in enumerate(critical_high[:20], 1):
            ...
            for cve in sorted(comp.cves, key=lambda c: c["cvss_score"], reverse=True)[
                :5
            ]:
...
def main():
    ...
    analyze_parser = subparsers.add_parser(
        "analyze", help="Analyze an SBOM for vulnerabilities"
    )
    analyze_parser.add_argument(
        "sbom_path", help="Path to SBOM file (CycloneDX or SPDX JSON)"
    )
    ...
    analyze_parser.add_argument(
        "--skip-nvd", action="store_true", help="Skip NVD API queries (offline mode)"
    )
    ...
    if args.command == "analyze":
        ...
        for comp in components:
            print(
                f"    {comp.name}@{comp.version} [{comp.component_type}] "
        ...
        for lic, count in sorted(
            info["license_distribution"].items(), key=lambda x: x[1], reverse=True
        ):
            print(f"  {lic}: {count}")
