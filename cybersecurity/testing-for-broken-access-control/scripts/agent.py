        ...
        for method in methods:
            ...
            try:
                resp = requests.request(
                    method, url, headers=headers, timeout=10, verify=False
                )
                if resp.status_code in (200, 201, 204):
                    ...
                    print(
                        f"  [!] VULNERABLE: {method} {endpoint} -> {resp.status_code}"
                    )
...
def test_horizontal_escalation(
    base_url, user_token, resource_templates, other_user_ids
):
        ...
        for uid in other_user_ids:
            ...
            try:
                ...
                if resp.status_code == 200 and len(resp.text) > 50:
                    ...
                    print(
                        f"  [!] IDOR: GET {url} -> {resp.status_code} ({len(resp.text)} bytes)"
                    )
            except requests.RequestException:
                continue
...
def test_method_override(base_url, user_token, endpoint):
    ...
    for oh in override_headers:
        for method in ["DELETE", "PUT", "PATCH"]:
            ...
            try:
                resp = requests.post(
                    url, headers=test_headers, timeout=10, verify=False
                )
                if resp.status_code in (200, 201, 204):
                    findings.append(
                        {
    ...
    if args.user_token:
        findings.extend(
            test_vertical_escalation(
                args.base_url, args.user_token, args.admin_endpoints
            )
        )
        findings.extend(
        ...
        findings.extend(
            test_method_override(
                args.base_url, args.user_token, args.admin_endpoints[0]
            )
        )
        findings.extend(
            test_mass_assignment(args.base_url, args.user_token, "/api/users/me")
        )
