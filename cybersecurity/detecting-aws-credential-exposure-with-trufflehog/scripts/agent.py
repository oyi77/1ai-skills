...
def scan_repo_trufflehog(repo_path, json_output=True):
    ...
    try:
                ...
                findings.append(
                    {
                        "detector": finding.get(
                            "DetectorName",
                            finding.get("SourceMetadata", {}).get("DetectorName"),
                        ),
        ...
        return {
            "error": "TruffleHog not installed. Install with: pip install trufflehog"
        }
...
if __name__ == "__main__":
    ...
    if action == "scan" and len(sys.argv) > 2:
        ...
        print(
            "Usage: agent.py [scan <path>|scan-org <org>|check-key <key_id>|deactivate <key_id> <user>|git-secrets]"
        )
