...
def list_safes(base_url, headers):
    ...
    for s in safes[:20]:
        print(
            f"  {s['safeName']} (retention: {s.get('numberOfDaysRetention', 'N/A')} days)"
        )
    return safes
...
def list_platforms(base_url, headers):
    ...
    for p in platforms:
        print(
            f"  {p.get('general', {}).get('name', 'Unknown')} - "
    ...
    for s in sessions:
        ...
        if duration > 3600:
            long_sessions.append(
                {
                    "user": s.get("User"),
                    "target": s.get("AccountName"),
                    "duration_sec": duration,
                    "severity": "MEDIUM",
                }
            )
            print(
                f"  [!] Long session: {s.get('User')} -> {s.get('AccountName')} "
                f"({duration // 60} min)"
            )
            ...
            findings.append(
                {
                    "account": a.get("name"),
                    "safe": a.get("safeName"),
                    "platform": a.get("platformId"),
                    "issue": "JIT not enabled",
                    "severity": "HIGH",
                }
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="CyberArk Zero Standing Privilege Audit Agent"
    )
    parser.add_argument(
        "action",
        choices=[
