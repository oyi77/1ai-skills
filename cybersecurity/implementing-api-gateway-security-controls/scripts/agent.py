        ...
        for resource in resources:
            for method_key in resource.get("resourceMethods", {}):
                method = client.get_method(
                    restApiId=api_id, resourceId=resource["id"], httpMethod=method_key
                )
...
def audit_kong_gateway(admin_url=None):
    ...
    for svc in services:
        ...
        plugins = (
            requests.get(f"{admin_url}/services/{svc_id}/plugins")
            .json()
            .get("data", [], timeout=30)
        )
        plugin_names = {p["name"] for p in plugins}
        if (
            "key-auth" not in plugin_names
            and "jwt" not in plugin_names
            and "oauth2" not in plugin_names
        ):
            findings.append(
                {
                    "service": svc_name,
                    "issue": "no_auth_plugin",
                    "severity": "CRITICAL",
                }
            )
        if (
            "rate-limiting" not in plugin_names
            and "rate-limiting-advanced" not in plugin_names
        ):
            findings.append(
                {
                    "service": svc_name,
...
def analyze_gateway_logs(log_path):
    ...
    for ip, count in sorted(ip_counts.items(), key=lambda x: -x[1])[:10]:
        if count > total * 0.3:
            findings.append(
                {
                    "issue": "traffic_concentration",
                    "ip": ip,
                    "request_pct": round(count / total * 100, 1),
                    "severity": "MEDIUM",
                }
            )
    return findings
...
def main():
    ...
    parser.add_argument(
        "--action", choices=["aws", "kong", "logs", "full"], default="full"
    )
    parser.add_argument(
        "--kong-url", default=os.environ.get("KONG_ADMIN_URL", "http://localhost:8001")
    )
