...
def launch_campaign(
    base_url, api_key, name, template_id, page_id, smtp_id, group_ids, url
):
...
def list_resources(base_url, api_key):
    ...
    return {
        ...
        "templates": [
            {"id": t["id"], "name": t["name"]} for t in client.get_templates()
        ],
        "sending_profiles": [
...
def generate_report(base_url, api_key, campaign_id):
    ...
    if metrics["rates"]["click_rate"] > 20:
        recommendations.append(
            "HIGH click rate — mandatory security awareness training needed"
        )
    ...
    if metrics["rates"]["report_rate"] < 5:
        recommendations.append(
            "Low report rate — train users on how to report phishing"
        )
    return {
        ...
        **metrics,
        "risk_level": (
            "CRITICAL"
            if metrics["rates"]["submit_rate"] > 10
            else "HIGH" if metrics["rates"]["click_rate"] > 20 else "MEDIUM"
        ),
