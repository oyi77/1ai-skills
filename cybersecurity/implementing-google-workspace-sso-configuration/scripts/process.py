...
class GoogleWorkspaceSSOValidator:
    ...
    def validate_idp_metadata(self, metadata_url):
        ...
        sso_elements = root.findall(".//md:IDPSSODescriptor/md:SingleSignOnService", ns)
        sso_urls = [
        ...
        return {
            ...
            "has_post_binding": any("HTTP-POST" in s["binding"] for s in sso_urls),
    ...
    def validate_sso_configuration(self, idp_sso_url, idp_entity_id, cert_pem):
        ...
        results["checks"].append(
            {
                ...
                "status": (
                    "PASS"
                    if cert_result.get("valid") and not cert_result.get("is_expired")
                    else "FAIL"
                ),
                "details": cert_result,
            }
        ...
        all_pass = all(c["status"] in ("PASS", "WARN") for c in results["checks"])
