            ...
            if len(domain_base) != len(known_base):
                ...
                for i, (a, b) in enumerate(zip(domain_base, known_base)):
                    if a != b:
                        if (
                            i > 0
                            and domain_base[i - 1 : i + 1] == known_base[i : i - 2 : -1]
                            if i > 0
                            else False
                        ):
...
def detect_impersonation(display_name: str, vip_names: list = None) -> list:
    ...
    for vip in vip_names:
        if vip.lower() in name_lower:
            findings.append(
                {"matched_vip": vip, "display_name": display_name, "confidence": 0.8}
            )
...
def analyze_email(
    headers: dict, body: str = "", vip_names: list = None, known_domains: list = None
) -> EmailAnalysis:
...
def generate_detection_rules(indicators_db: list) -> str:
    ...
    for ind_type, indicators in by_type.items():
        values = list(
            set(ind.get("raw_value", "") for ind in indicators if ind.get("raw_value"))
        )
        if not values:
            continue
        ...
        rule = {
            ...
            "action": (
                "quarantine"
                if indicators[0].get("severity") in ("high", "critical")
                else "tag"
            ),
    ...
    output_lines = [
        "# Auto-generated Spearphishing Detection Rules",
...
def format_analysis_report(analysis: EmailAnalysis) -> str:
    ...
    lines.append(
        f"  Verdict: {'SPEARPHISHING DETECTED' if analysis.is_spearphishing else 'NOT DETECTED'}"
    )
    lines.append("")
    ...
    if analysis.domain_similarities:
        ...
        for sim in analysis.domain_similarities:
            lines.append(
                f"  {sim.suspicious_domain} ~ {sim.original_domain} "
...
def main():
    ...
    detect_parser = subparsers.add_parser(
        "detect", help="Detect spearphishing in single email"
    )
    ...
    domain_parser = subparsers.add_parser(
        "check-domain", help="Check domain similarity"
    )
    domain_parser.add_argument("domain", help="Domain to check")

    rules_parser = subparsers.add_parser(
        "rules", help="Generate detection rules from log"
    )
    ...
    if args.command == "analyze":
        ...
        print(
            f"Analyzed {len(results)} emails, detected {spearphishing_count} spearphishing attempts"
        )
        print()
