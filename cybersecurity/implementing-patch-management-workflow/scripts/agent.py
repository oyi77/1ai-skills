    ...
    if result.returncode == 0 and result.stdout.strip():
        try:
            ...
            if hotfixes:
                ...
                print(
                    f"[+] Latest patch: {latest.get('HotFixID', 'N/A')} ({installed_date})"
                )
...
def main():
    ...
    parser.add_argument(
        "--platform",
        choices=["auto", "apt", "yum", "windows"],
        default="auto",
        help="Package manager to check",
    )
    parser.add_argument("--cves", nargs="+", help="CVE IDs to check against KEV")
    parser.add_argument(
        "--sla-critical", type=int, default=7, help="SLA days for critical (default: 7)"
    )
    parser.add_argument(
        "--sla-high", type=int, default=30, help="SLA days for high (default: 30)"
    )
    ...
    report = {
        ...
        "risk_level": (
            "CRITICAL"
            if kev_findings or severity_counts.get("CRITICAL", 0) > 0
            else (
                "HIGH"
