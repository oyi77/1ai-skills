...
def get_binauthz_policy(project):
    ...
    return run_gcloud(
        ["container", "binauthz", "policy", "export", "--project", project]
    )
...
def list_attestors(project):
    ...
    return run_gcloud(
        ["container", "binauthz", "attestors", "list", "--project", project]
    )
...
def audit_policy(policy):
    ...
    if eval_mode == "ALWAYS_ALLOW":
        findings.append(
            {
                "issue": "Default rule allows all images",
                "severity": "CRITICAL",
    ...
    if enforce_mode == "DRYRUN_AUDIT_LOG_ONLY":
        findings.append(
            {
                "issue": "Default rule is dry-run only",
                "severity": "HIGH",
    ...
    if eval_mode == "REQUIRE_ATTESTATION" and not attestors:
        findings.append(
            {
                "issue": "Attestation required but no attestors configured",
                "severity": "CRITICAL",
            }
        )
    ...
    if global_eval != "ENABLE":
        findings.append(
            {
                "issue": "Global policy evaluation not enabled",
                "severity": "MEDIUM",
    ...
    for pattern in whitelist:
        ...
        if name.endswith("/**") or name.endswith("/*"):
            if not any(
                safe in name for safe in ["gcr.io/google", "k8s.gcr.io", "gke.gcr.io"]
            ):
                findings.append(
                    {
                        "issue": f"Broad whitelist pattern: {name}",
                        "severity": "HIGH",
    ...
    for cluster, rule in cluster_rules.items():
        if rule.get("evaluationMode") == "ALWAYS_ALLOW":
            findings.append(
                {"issue": f"Cluster {cluster} allows all images", "severity": "HIGH"}
            )
...
def main():
    ...
    parser.add_argument(
        "--action",
        choices=["audit", "list-attestors", "verify", "generate", "cv-status"],
        default="audit",
    )
    ...
    if args.action == "audit":
        ...
        print(
            f"[+] Found {len(attestors) if isinstance(attestors, list) else 0} attestors"
        )
