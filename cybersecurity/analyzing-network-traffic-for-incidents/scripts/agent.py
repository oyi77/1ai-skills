...
def extract_dns_queries(pcap_path):
    ...
    queries = run_tshark(
        pcap_path, "dns.qr==0", ["ip.src", "dns.qry.name", "dns.qry.type"]
    )
    return queries
...
def detect_ids_alerts(pcap_path):
    ...
    suricata_output = os.environ.get(
        "SURICATA_OUTPUT_DIR", os.path.join(tempfile.gettempdir(), "suricata_output")
    )
...
if __name__ == "__main__":
    ...
    if pcap and os.path.exists(pcap):
        ...
        for b in beacons:
            print(
                f"  [!] {b['flow']}: interval={b['avg_interval']}s "
