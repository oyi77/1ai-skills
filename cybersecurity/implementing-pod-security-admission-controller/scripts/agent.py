...
def get_namespaces():
    ...
    for ns in data.get("items", []):
        ...
        psa_enforce_version = labels.get(
...
def format_summary(ns_findings, pod_findings, namespaces):
    ...
    for ns in namespaces:
        ...
        print(
            f"    {ns['name']:30s}: enforce={status:12s} audit={ns['audit'] or 'none':12s}"
        )
    ...
    if pod_findings:
        ...
        for f in pod_findings[:15]:
            if f["severity"] in ("CRITICAL", "HIGH"):
                print(
                    f"    [{f['severity']:8s}] {f.get('namespace', '')}/"
                    f"{f.get('pod', '')}: {f['check']}"
                )
...
def main():
    ...
    parser.add_argument(
        "--namespace", "-n", help="Specific namespace to audit (default: all)"
    )
    parser.add_argument(
        "--skip-pods", action="store_true", help="Only audit namespace labels"
    )
