...
def identify_priority_gaps(tactic_coverage, priority_techniques=None):
    ...
    for tactic_id, data in tactic_coverage.items():
        for tech in data["gaps"]:
            priority = (
                "high"
                if (priority_techniques and tech in priority_techniques)
                else "medium"
            )
            gaps.append(
                {
    ...
    return sorted(
        gaps, key=lambda x: (0 if x["priority"] == "high" else 1, x["tactic"])
    )
...
def calculate_detection_depth(technique_rules):
    ...
    for tech, rules in technique_rules.items():
        ...
        data_sources = list(
            set(r["data_source"] for r in enabled_rules if r["data_source"])
        )
        depth[tech] = {
            ...
            "depth": (
                ...
                else "moderate" if len(enabled_rules) >= 2 else "shallow"
            ),
        }
    return depth
...
def main():
    ...
    parser.add_argument(
        "--rules",
        required=True,
        help="JSON file with detection rules and ATT&CK mappings",
    )
    ...
    attack_matrix = (
        load_attack_matrix(args.matrix)
        if args.matrix
        else {t: [] for t in ENTERPRISE_TACTICS}
    )
    ...
    logger.info(
        "Coverage: %d techniques covered by %d rules", len(technique_rules), len(rules)
    )
