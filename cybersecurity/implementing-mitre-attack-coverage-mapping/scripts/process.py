...
class CoverageMap:
    ...
    def get_coverage_summary(self) -> dict:
        ...
        covered = sum(
            ...
            if t in self.technique_scores and self.technique_scores[t]["max_score"] > 0
        )
        no_coverage = total_techniques - covered

        scores = [
            self.technique_scores.get(t, {}).get("max_score", 0)
            for t in ENTERPRISE_TECHNIQUES
        ]
    ...
    def get_tactic_coverage(self) -> dict:
        ...
        for tactic_id, tactic_name in ATTACK_TACTICS.items():
            techniques_in_tactic = [
                t
                for t, info in ENTERPRISE_TECHNIQUES.items()
                if info["tactic"] == tactic_id
            ]
            covered = sum(
                1
                for t in techniques_in_tactic
                if t in self.technique_scores
                and self.technique_scores[t]["max_score"] > 0
            )
                ...
                gaps.append(
                    {
                        ...
                        "rules_count": len(
                            self.technique_scores.get(tech_id, {}).get("rules", [])
                        ),
                    }
                )
    ...
    def generate_navigator_layer(self) -> dict:
        ...
        for tech_id, info in ENTERPRISE_TECHNIQUES.items():
            ...
            color = (
                "#ff0000"
                if score == 0
                else "#ffff00" if score < 50 else "#90ee90" if score < 75 else "#00ff00"
            )
            techniques.append(
                {
                    "techniqueID": tech_id,
                    ...
                    "score": score,
                    "comment": (
                        f"{len(rules)} rules: {', '.join(rules[:3])}"
                        if rules
                        else "NO DETECTION"
                    ),
                }
...
if __name__ == "__main__":
    ...
    rules = [
        DetectionRule(
            "Brute Force Detection", ["T1110"], 85, ["Windows Security Log"], True
        ),
        DetectionRule(
            "Suspicious PowerShell", ["T1059"], 75, ["PowerShell Script Block"], True
        ),
        DetectionRule(
            "New Account Created", ["T1136"], 60, ["Windows Security Log"], False
        ),
        DetectionRule(
            "Lateral Movement RDP",
            ["T1021"],
            70,
            ["Windows Security Log", "Firewall"],
            True,
        ),
        DetectionRule(
            "Phishing Email Detected", ["T1566"], 80, ["Email Gateway"], True
        ),
        DetectionRule("Credential Dumping", ["T1003"], 50, ["Sysmon"], False),
        DetectionRule(
            "Scheduled Task Created", ["T1053"], 65, ["Windows Security Log"], True
        ),
    ...
    print(
        f"Coverage: {summary['covered']}/{summary['total_techniques']} techniques ({summary['coverage_pct']}%)"
    )
    ...
    for tactic, data in cmap.get_tactic_coverage().items():
        if data["total"] > 0:
            ...
            print(
                f"{tactic:<30} {data['covered']:<10} {data['total']:<8} [{bar}] {data['pct']}%"
            )

    print(f"\nDetection Gaps (Score < 50):")
    for gap in cmap.get_gaps():
        print(
            f"  [{gap['current_score']:>3}] {gap['technique_id']} - {gap['technique_name']} ({gap['tactic']})"
        )
