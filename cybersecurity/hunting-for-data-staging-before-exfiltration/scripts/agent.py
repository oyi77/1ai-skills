...
def detect_archive_creation(events):
    ...
    for event in events:
        ...
        output_files = re.findall(
            r"[\w\-./\\]+\.(zip|7z|rar|tar\.gz|tar\.bz2|tar|cab)", cmdline
        )
        encrypted = any(
            flag in cmdline for flag in ["-p", "-hp", "-mhe=on", "--encrypt", "-e"]
        )
        ...
        detections.append(
            {
                ...
                "parent_process": event.get(
                    "ParentImage", event.get("ParentProcessName", "")
                ),
                "severity": severity,
        ...
        for staging_path in all_staging:
            if target.startswith(staging_path.lower()):
                staging_writes.append(
                    {
                        "timestamp": event.get("UtcTime", event.get("TimeCreated", "")),
                        "target_path": event.get(
                            "TargetFilename", event.get("ObjectName", "")
                        ),
...
def generate_report(archives, staging_writes, consolidation, log_path):
    ...
    return {
        ...
        "risk_level": (
            "critical"
            if risk_score >= 70
            else "high" if risk_score >= 40 else "medium" if risk_score >= 20 else "low"
        ),
...
def main():
    parser = argparse.ArgumentParser(
        description="Data Staging Detection Agent (MITRE T1074)"
    )
    ...
    parser.add_argument(
        "--read-threshold", type=int, default=20, help="Bulk read threshold"
    )
    ...
    print(
        f"[+] Archive creation: {len(archives)} | Staging writes: {len(staging_writes)} | Consolidation: {len(consolidation)}"
    )
