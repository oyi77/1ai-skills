...
def create_event_from_data(misp, event_data):
    ...
    event.distribution = event_data.get(
        "distribution", 1
    )  # 0=org, 1=community, 2=connected, 3=all
    event.threat_level_id = event_data.get(
        "threat_level", 2
...
def validate_event_quality(event_data):
    ...
    has_mitre = any(
        "mitre-attack" in t.lower() or "attack-pattern" in t.lower() for t in tag_names
    )
    ...
    for attr in attrs:
        val = attr.get("value", "")
        atype = attr.get("type", "")
        if atype in ("ip-dst", "ip-src") and val in (
            "127.0.0.1",
            "0.0.0.0",
            "10.0.0.1",
            "192.168.1.1",
...
def main():
    ...
    ap.add_argument(
        "--input", required=True, help="Input JSON with MISP events or config"
    )
