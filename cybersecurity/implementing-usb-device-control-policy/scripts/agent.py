...
def audit_linux_usbguard():
    ...
    if result.stdout.strip() == "active":
        findings.append(
            {
                "check": "USBGuard service",
                "status": "PASS",
                "severity": "INFO",
                "detail": "USBGuard is running",
            }
        )
    else:
        findings.append(
            {
                ...
                "status": "FAIL",
                "severity": "HIGH",
                "detail": "USBGuard is not running",
            }
        )
        return findings
        ...
        for line in result.stdout.strip().splitlines():
            ...
            if len(parts) >= 3:
                ...
                if policy == "allow":
                    findings.append(
                        {
                            "check": f"USB Device {dev_id}",
                            "status": "INFO",
                            "severity": "INFO",
                            "detail": f"Allowed: {name}",
                        }
                    )
                elif policy == "block":
                    findings.append(
    ...
    if result.returncode == 0:
        ...
        if policy == "block":
            findings.append(
                {
                    "check": "Default USB policy",
                    "status": "PASS",
                    "severity": "INFO",
                    "detail": "Default: block (deny by default)",
                }
            )
        else:
            findings.append(
        ...
        findings.append(
            {
                ...
                "status": "INFO",
                "severity": "INFO",
                "detail": f"{len(rules)} rules configured",
            }
        )
...
def audit_linux_udev():
        ...
        for fname in os.listdir(udev_dir):
            ...
            try:
                with open(fpath, "r") as f:
                    content = f.read()
                if "usb" in content.lower() and (
                    "authorize" in content.lower() or "block" in content.lower()
                ):
                    usb_rules_found = True
                    findings.append(
                        {
                            "check": f"udev USB rule: {fname}",
                            "status": "INFO",
                            "severity": "INFO",
                            "detail": fpath,
                        }
    ...
    if not usb_rules_found:
        findings.append(
            {
                "check": "udev USB rules",
                "status": "WARN",
                "severity": "MEDIUM",
                "detail": "No USB-specific udev rules found",
            }
        )
    return findings
...
def format_summary(findings, devices):
    ...
    if devices:
        ...
        for d in devices:
            print(
                f"    {d.get('name', 'Unknown'):40s} | {d.get('id', d.get('instance_id', 'N/A'))}"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="USB device control policy audit agent"
    )
    parser.add_argument(
        "--list-devices", action="store_true", help="List connected USB devices"
    )
