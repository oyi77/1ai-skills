...
class PacketInjectionAgent:
    ...
    def test_tcp_syn(self, port=80):
        ...
        if resp and resp.haslayer(TCP):
            ...
            state = (
                "open" if flags == "SA" else "closed" if flags == "RA" else str(flags)
            )
            return self._record_result(
                "TCP SYN", "port_scan", True, {"port": port, "state": state}
            )
        return self._record_result(
            "TCP SYN", "port_scan", True, {"port": port, "state": "filtered"}
        )
    ...
    def test_land_attack(self, port=80):
        ...
        pkt = IP(src=self.target_ip, dst=self.target_ip) / TCP(
    ...
    def test_fragmentation_overlap(self, port=80):
        ...
        frag1 = (
            IP(dst=self.target_ip, flags="MF", frag=0)
