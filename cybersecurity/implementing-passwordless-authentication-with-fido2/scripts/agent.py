...
def audit_fido2_policy(policy):
    ...
    if policy.get("state") != "enabled":
        findings.append(
            {
                "issue": f"FIDO2 policy state: {policy.get('state', 'unknown')}",
                "severity": "high",
            }
        )
...
def analyze_adoption(registrations):
    ...
    total = len(users)
    fido2 = sum(
        1 for u in users if "fido2" in str(u.get("methodsRegistered", [])).lower()
    )
    passwordless = sum(1 for u in users if u.get("isPasswordlessCapable", False))
    return {
        "total_users": total,
        "fido2_registered": fido2,
...
def check_rp_config(rp_url):
    ...
    try:
        ...
        config = {}
        findings.append(
            {"issue": "WebAuthn well-known not configured", "severity": "high"}
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="FIDO2 Passwordless Authentication Audit Agent"
    )
    ...
    logger.info(
        "FIDO2: adoption %.1f%%, %d findings",
        adoption["fido2_adoption_rate"],
