...
def generate_assessment_template(output_file=None):
    ...
    return {
        ...
        "standards": list(NERC_CIP_STANDARDS.keys()),
        "output": output_file,
    }
...
def assess_compliance(assessment_csv):
    ...
    gaps = [
        ...
        in ("fail", "non-compliant", "no", "not met", "gap")
    ]
    return {
...
def assess_esp_security(firewall_csv):
    ...
    has_default_deny = any(
        r.get("action", "").lower() in ("deny", "drop")
        and r.get("source", "").lower() in ("any", "0.0.0.0/0")
        for r in rules
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Power Grid Cybersecurity Assessment Agent (NERC CIP)"
    )
