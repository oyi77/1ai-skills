...
class CyberArkPAMAuditor:
    ...
    def _audit_stale_accounts(self):
        ...
        for acct in self.accounts:
            ...
            if not acct.last_access:
                stale.append(
                    f"{acct.username}@{acct.address}: Never accessed from vault"
                )
            else:
                try:
        ...
        for safe in self.safes:
            if not safe.dual_control:
                high_priv_types = any(
                    ...
                    for acct in self.accounts
                    if acct.safe_name == safe.safe_name
                )
                if high_priv_types:
                    self.findings.append(
                        PAMAuditFinding(
    ...
    def _audit_service_account_dependencies(self):
        ...
        service_accounts = [
            a for a in self.accounts if a.account_type == "service_account"
        ]
        if service_accounts:
            rapid_rotation = [
                a for a in service_accounts if a.rotation_interval_days < 7
            ]
            if rapid_rotation:
                self.findings.append(
                    PAMAuditFinding(
    ...
    def generate_report(self) -> str:
        ...
        overall = (
            ...
            if critical_count > 0
                ...
                "PASS WITH FINDINGS"
                if len(self.findings) > len(by_severity.get("info", []))
                else "PASS"
            )
        )
...
def main():
    ...
    sample_accounts = [
        {
            "account_id": "1",
            "username": "domain_admin01",
            "address": "dc01.corp.local",
            "platform_id": "WinDomain",
            "safe_name": "DomainAdmins",
            "account_type": "domain_admin",
            "last_rotation": "2026-02-20",
            "last_verification": "2026-02-22",
            "last_access": "2026-02-21",
        },
        {
            "account_id": "2",
            "username": "root",
            "address": "webserver01.corp.local",
            "platform_id": "UnixSSH",
            "safe_name": "LinuxRoot",
            "account_type": "root",
            "last_rotation": "2026-01-15",
            "last_verification": "2026-01-16",
        },
        {
            "account_id": "3",
            "username": "sa",
            "address": "sqlserver01.corp.local",
            "platform_id": "MSSQL",
            "safe_name": "DatabaseAdmins",
            "account_type": "dba",
            "last_rotation": "2026-02-22",
            "last_verification": "2026-02-22",
            "last_access": "2026-02-23",
        },
        {
            "account_id": "4",
            "username": "svc_backup",
            "address": "backup01.corp.local",
            "platform_id": "WinService",
            "safe_name": "ServiceAccounts",
            "account_type": "service_account",
            "last_rotation": "2026-02-01",
            "rotation_interval_days": 5,
        },
        {
            "account_id": "5",
            "username": "admin",
            "address": "switch01.corp.local",
            "platform_id": "CiscoIOS",
            "safe_name": "NetworkDevices",
            "account_type": "network",
            "last_rotation": "2026-02-10",
            "last_verification": "2026-02-10",
        },
    ]
    ...
    sample_safes = [
        {
            "safe_name": "DomainAdmins",
            "description": "Domain admin accounts",
            "dual_control": True,
            "members": [
        ...
        {
            "safe_name": "LinuxRoot",
            "description": "Linux root accounts",
            "dual_control": False,
            "members": [
                {"user": "admin1", "role": "admin"},
                {"user": "admin2", "role": "admin"},
                {"user": "admin3", "role": "admin"},
                {"user": "admin4", "role": "admin"},
            ],
            "account_count": 20,
        },
        {
            ...
            "dual_control": False,
            "members": [{"user": "svc_team", "role": "full_access"}],
            "account_count": 50,
        },
    ]
