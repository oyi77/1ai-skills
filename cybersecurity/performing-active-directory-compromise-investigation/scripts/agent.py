...
COMPROMISE_INDICATORS = {
    4662: {
        ...
        "description": "Directory service object accessed with replication rights",
    },
    4769: {
        "name": "Kerberoasting",
        "description": "Kerberos service ticket requested with RC4 encryption",
    },
    ...
    4624: {
        "name": "Logon Event",
        "description": "Successful logon - check Type 3 for lateral movement",
    },
    4648: {
...
def parse_evtx_json(log_path: str) -> list[dict]:
    ...
    try:
        ...
        events = [
            json.loads(line) for line in content.strip().splitlines() if line.strip()
        ]
    return events
...
def main():
    ...
    parser.add_argument(
        "--log", required=True, help="Path to JSON-exported Windows Security event log"
    )
