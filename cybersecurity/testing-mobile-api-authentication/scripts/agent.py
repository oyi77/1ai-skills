...
class MobileAPIAuthAgent:
    ...
    def test_no_auth_access(self, endpoints=None):
        ...
        for ep in targets:
            ...
            if resp and resp.status_code == 200:
                results.append(
                    {"endpoint": ep, "status": 200, "body_len": len(resp.text)}
                )
    ...
    def test_rate_limiting(self, login_path="/api/v1/login", attempts=20):
        ...
        for i in range(attempts):
            resp = self._req(
                "POST", login_path, json={"username": "test", "password": f"wrong{i}"}
            )
    ...
    def test_idor(
        self, token, resource_path="/api/v1/users/{id}", own_id="1", other_id="2"
    ):
