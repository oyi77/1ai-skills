...
def aws_snapshot_ebs(instance_id):
    ...
    for vol_id in volume_ids:
        ...
        snap_result = subprocess.run(
            snap_cmd, capture_output=True, text=True, timeout=120
        )
        if snap_result.returncode == 0:
            snap_data = json.loads(snap_result.stdout)
            snapshots.append(snap_data.get("SnapshotId"))
            logger.info(
                "Created snapshot %s for volume %s", snap_data.get("SnapshotId"), vol_id
            )
    return snapshots
...
def aws_query_cloudtrail(username, hours_back=24):
    ...
    if result.returncode == 0:
        ...
        logger.info(
            "CloudTrail: %d events for user %s in last %d hours",
            len(events),
            username,
            hours_back,
        )
...
def aws_check_all_regions_instances():
    ...
    for region in regions:
        ...
        if r.returncode == 0:
            ...
            running = [
                i for reservation in instances for i in reservation if i[2] == "running"
            ]
            if running:
...
def main():
    ...
    parser.add_argument(
        "--forensic-sg", default="sg-forensic-isolate", help="Forensic SG ID"
    )
    ...
    report = generate_ir_report(
        args.incident_id, args.username, events, snapshots, containment
    )
