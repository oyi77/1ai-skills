...
def detect_suspicious_activity(client, hours=24):
    ...
    for event_name, description in SUSPICIOUS_EVENTS.items():
        ...
        for e in events:
            ...
            severity = (
                "CRITICAL"
                if event_name in ["StopLogging", "DeleteTrail", "DisableKey"]
                else (
                    "HIGH"
                    if event_name
                    in ["CreateUser", "CreateAccessKey", "AttachUserPolicy"]
                    else "MEDIUM"
                )
            )
            detections.append(
...
def detect_failed_auth(client, hours=24):
    ...
    events = lookup_events(
        client, event_name="ConsoleLogin", hours=hours, max_results=100
    )
    ...
    return {
        ...
        "by_source_ip": dict(by_ip.most_common(10)),
...
def detect_unauthorized_regions(client, authorized_regions, hours=24):
    ...
    unauthorized = [
        ...
        for e in events
        if e.get("region")
        and e["region"] not in authorized_regions
        and not e.get("error")
    ]
    return unauthorized
...
def main():
    ...
    parser.add_argument(
        "--hours", type=int, default=24, help="Lookback period in hours"
    )
