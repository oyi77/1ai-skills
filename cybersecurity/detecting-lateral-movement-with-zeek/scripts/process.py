...
def detect_admin_shares(log_file, internal_nets):
    ...
    for entry in entries:
        ...
        if any(s in share_upper for s in ["ADMIN$", "C$", "IPC$"]):
            severity = (
                "HIGH" if "ADMIN$" in share_upper or "C$" in share_upper else "MEDIUM"
            )
            print(
                f"[{severity}] ADMIN SHARE: {entry.get('ts', '')} {src} -> {dst} ({share})"
            )
...
def detect_rdp_pivots(log_file, window_minutes=10):
    ...
    entries = parse_zeek_log(log_file)
    rdp_sessions = [
        (float(e.get("ts", 0)), e.get("id.orig_h", ""), e.get("id.resp_h", ""))
        for e in entries
        if e.get("id.resp_p") == "3389"
    ]
    rdp_sessions.sort()
    ...
    for ts, src, dst in rdp_sessions:
        for arrival_ts, arrival_src in dst_arrivals.get(src, []):
            if 0 < (ts - arrival_ts) < window_minutes * 60:
                print(
                    f"[HIGH] RDP PIVOT: {arrival_src} -> {src} -> {dst} (delta: {int(ts - arrival_ts)}s)"
                )
    ...
    for user, events in user_events.items():
        ...
        for i, (ts_start, _) in enumerate(events):
            ...
            if len(window_hosts) >= threshold:
                print(
                    f"[CRITICAL] NTLM ACCOUNT SPRAY: {user} authenticated to {len(window_hosts)} "
...
def detect_dcsync(log_file, dc_ips=None):
    ...
    if not dc_ips:
        print(
            "[WARN] DCSync detection skipped: --dc-ips not provided. "
...
if __name__ == "__main__":
    ...
    if log_type == "smb_mapping":
        ...
        detect_ntlm_spray(
            log_file, int(args.get("--window", 300)), int(args.get("--threshold", 3))
        )
