...
NERC_CIP_CONTROLS = {
    "CIP-002": {
        ...
        "checks": [
            ...
            "impact_ratings_assigned",
            "annual_review_documented",
        ],
    },
    "CIP-003": {
        "name": "Security Management Controls",
        "checks": [
            "cyber_security_policy_exists",
            "senior_manager_designated",
    ...
    "CIP-006": {
        ...
        "checks": [
            ...
            "visitor_control_program",
            "psp_access_monitoring",
        ],
    },
    "CIP-007": {
        "name": "System Security Management",
        "checks": [
            "ports_services_minimized",
    ...
    "CIP-009": {
        "name": "Recovery Plans",
        "checks": [
            "recovery_plans_exist",
            "backup_procedures",
            "recovery_testing_annual",
        ],
    },
    "CIP-010": {
    ...
    "CIP-011": {
        ...
        "checks": ["information_classification", "bcsi_protection", "bcsi_disposal"],
    },
    "CIP-013": {
    ...
    if target_ip:
        ...
        if open_ports > 10:
            findings.append(
                {
                    "control": "CIP-005",
                    "issue": f"{open_ports} open ports on ESP boundary",
                    "severity": "high",
                }
            )
    return findings
...
def check_system_hardening():
    ...
    if service_count > 50:
        findings.append(
            {
                "control": "CIP-007-R1",
                "issue": f"{service_count} running services (minimize unused)",
                "severity": "medium",
            }
        )
    patch_cmd = (
        ["apt", "list", "--upgradable"]
        if subprocess.run(["which", "apt"], capture_output=True, timeout=120).returncode
        == 0
        else ["yum", "check-update"]
    )
    ...
    if pending > 0:
        findings.append(
            {
                "control": "CIP-007-R2",
                "issue": f"{pending} pending security patches",
                "severity": "high",
            }
        )
    return findings
...
def main():
    ...
    parser.add_argument(
        "--evidence-file", help="JSON evidence file with control assessment results"
    )
    parser.add_argument("--target-ip", help="ESP boundary IP to scan")
    parser.add_argument(
        "--skip-hardening", action="store_true", help="Skip system hardening checks"
    )
    ...
    logger.info(
        "NERC CIP compliance: %.1f%% (%d/%d), %d findings",
        report["overall_compliance_rate"],
        report["total_passed"],
        report["total_controls_assessed"],
        len(report["technical_findings"]),
    )
