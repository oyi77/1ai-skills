    ...
    for pattern in PHISHING_INFRA_PATTERNS:
        if re.search(pattern, url_lower):
            finding.indicators.append(
                f"Known phishing infrastructure pattern: {pattern}"
            )
            score += 25
...
def scan_email_content(eml_content: str) -> QuishingAnalysis:
    ...
    has_images = bool(
        re.search(r"<img|Content-Type:\s*image/", eml_content, re.IGNORECASE)
    )
...
def format_report(analysis: QuishingAnalysis) -> str:
    ...
    if analysis.findings:
        ...
        for i, finding in enumerate(analysis.findings, 1):
            ...
            lines.append(
                f"       Suspicious: {'YES' if finding.is_suspicious else 'No'}"
            )
...
def main():
    ...
    if args.command == "scan-image":
        ...
        analysis = QuishingAnalysis(
            source_file=args.image, qr_codes_found=len(findings), findings=findings
        )
        if args.json:
