...
def audit_zone_architecture(zones_path):
    ...
    for zone in zones:
        ...
        if not sl_target:
            findings.append(
                {"zone": zone_name, "issue": "No SL-T defined", "severity": "CRITICAL"}
            )
    ...
    for conduit in conduits:
        ...
        if conduit.get("allows_remote_access", False) and not conduit.get(
            "requires_mfa", False
        ):
            findings.append(
                {
...
def main():
    ...
    parser.add_argument(
        "--action", choices=["audit", "template", "sl-map", "full"], default="full"
    )
    ...
    if args.action in ("template", "full") and args.facility:
        ...
        print(
            f"[+] Template: {len(template['zones'])} zones, {len(template['conduits'])} conduits"
        )
