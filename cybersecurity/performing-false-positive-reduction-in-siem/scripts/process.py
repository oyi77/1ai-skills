...
class AlertAnalyzer:
    ...
    def get_fp_rates(self) -> list:
        ...
        for rule, stats in rule_stats.items():
            ...
            avg_triage = round(
                sum(stats["triage_times"]) / max(1, len(stats["triage_times"])), 1
            )
            results.append(
                {
                    "rule_name": rule,
    ...
    def get_overall_metrics(self) -> dict:
        ...
        return {
            ...
            "total_analyst_hours_wasted_on_fp": round(
                sum(
                    a["triage_time_sec"]
...
if __name__ == "__main__":
    ...
    for rule_name, fp_prob in rules:
        for _ in range(random.randint(20, 80)):
            classification = (
                ...
                else ("tp" if random.random() > 0.3 else "btp")
            )
            analyzer.add_alert(
                rule_name,
                classification,
    ...
    print(f"\nPer-Rule FP Rates:")
    print(
        f"{'Rule':<35} {'Total':<8} {'FP':<6} {'TP':<6} {'FP Rate':<10} {'Precision':<10} {'Needs Tuning'}"
    )
    print("-" * 90)
    for r in analyzer.get_fp_rates():
        flag = "*** YES ***" if r["needs_tuning"] else "No"
        print(
            f"{r['rule_name']:<35} {r['total']:<8} {r['false_positives']:<6} {r['true_positives']:<6} {r['fp_rate']:<10} {r['precision']:<10} {flag}"
        )

    print(f"\nTuning Recommendations:")
    for rec in analyzer.generate_recommendations():
        print(
            f"  [{rec['priority'].upper()}] {rec['rule']}: {rec['action']} (FP: {rec['fp_rate']}%)"
        )
