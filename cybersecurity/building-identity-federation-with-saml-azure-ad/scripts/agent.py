...
def parse_metadata(xml_text):
    ...
    if idp_desc is not None:
        for sso in idp_desc.findall("md:SingleSignOnService", ns):
            sso_services.append(
                {"binding": sso.get("Binding"), "location": sso.get("Location")}
            )
    certs = []
...
def validate_configuration(idp_metadata, sp_config):
    ...
    if not idp_metadata.get("sso_services"):
        findings.append(
            {"issue": "No SSO services in IdP metadata", "severity": "critical"}
        )
    if not idp_metadata.get("certificates"):
        findings.append(
            {"issue": "No signing certificates in metadata", "severity": "critical"}
        )
    if (
        not sp_config.get("assertionConsumerService", {})
        .get("location", "")
        .startswith("https://")
    ):
        findings.append({"issue": "ACS URL not using HTTPS", "severity": "high"})
    http_redirect = any(
        "HTTP-Redirect" in s.get("binding", "")
        for s in idp_metadata.get("sso_services", [])
    )
    http_post = any(
        "HTTP-POST" in s.get("binding", "")
        for s in idp_metadata.get("sso_services", [])
    )
    if not http_post:
        findings.append(
            {"issue": "HTTP-POST binding not available", "severity": "medium"}
        )
    return {
...
def main():
    ...
    parser.add_argument(
        "--sp-entity-id", required=True, help="Service Provider entity ID"
    )
    parser.add_argument(
        "--acs-url", required=True, help="Assertion Consumer Service URL"
    )
