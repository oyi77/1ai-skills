...
def run_grype_scan(
    image: str, output_format: str = "json", fail_on: str = None
) -> dict:
...
def gate_check(vulns: list, max_critical: int = 0, max_high: int = 0) -> bool:
    ...
    if summary["Critical"] > max_critical:
        print(
            f"GATE FAIL: {summary['Critical']} critical vulnerabilities (max: {max_critical})"
        )
        passed = False
...
def main():
    parser = argparse.ArgumentParser(
        description="Grype Container Image Scanner Utility"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Max allowed critical vulns (default: 0)",
    )
    parser.add_argument(
