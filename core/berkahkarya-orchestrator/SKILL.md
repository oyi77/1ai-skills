...
description: Master orchestrator for BerkahKarya AI workforce. Routes tasks to the correct skill division, coordinates multi-skill workflows, manages parallel execution, and maintains task state. Use when: (1) task spans multiple skills/divisions, (2) unsure which skill to use, (3) complex multi-step business workflows, (4) need parallel agent coordination. NOT for: simple single-skill tasks (just use the skill directly).
---
