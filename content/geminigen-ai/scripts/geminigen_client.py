...
class GeminiGenClient:
    ...
    def _check_rate_limit(self, model: str = "nano-banana-pro") -> None:
        ...
        if recent_minute >= self.RATE_LIMIT_PER_MINUTE:
            ...
            logger.warning(
                f"Rate limit: {recent_minute}/min. Waiting {wait_time:.1f}s..."
            )
            time.sleep(wait_time + 1)

        if recent_hour >= self.RATE_LIMIT_PER_HOUR:
            raise RateLimitError(
                f"Hourly rate limit exceeded ({recent_hour}/{self.RATE_LIMIT_PER_HOUR})"
            )
            ...
            try:
                ...
                if response.status_code >= 500:
                    if attempt < max_retries:
                        wait = 2**attempt
                        logger.warning(
                            f"Server error {response.status_code}. Retry in {wait}s..."
                        )
                        time.sleep(wait)
                        continue
    ...
    def _poll_for_result(
        self, uuid: str, timeout: int = 300, poll_interval: int = 5
    ) -> GenerationResult:
        """
        ...
        while time.time() - start_time < timeout:
            ...
            if status == GenerationStatus.COMPLETED:
                ...
                result_url = (
                    data.get("generate_result")
                    or data.get("generated_image", [{}])[0].get("image_url")
                    or data.get("generated_video", [{}])[0].get("video_url")
                    or data.get("generated_audio", [{}])[0].get("audio_url")
                )
    ...
    def generate_image(
        ...
        response = self._request_with_retry("POST", "/generate_image", data=data)
        ...
        return GenerationResult(
            uuid=uuid, status=GenerationStatus.PROCESSING, raw_response=result
        )
    ...
    def generate_video_veo(
        ...
        response = self._request_with_retry("POST", "/video-gen/veo", data=data)
    ...
    def generate_video_sora(
        ...
        response = self._request_with_retry("POST", "/video-gen/sora", data=data)
    ...
    def generate_video_grok(
        ...
        response = self._request_with_retry("POST", "/video-gen/grok", data=data)
    ...
    def generate_tts(
        ...
        payload = {
            "model": model,
            "voices": [
                {"name": voice_name, "voice": {"id": voice_id, "name": voice_name}}
            ],
            "speed": speed,
            "input": text,
            "output_format": output_format,
        ...
        response = self._request_with_retry("POST", "/text-to-speech", json=payload)
    ...
    def download_result(self, url: str, output_path: str, timeout: int = 120) -> str:
        """
...
def main():
    ...
    parser.add_argument(
        "action",
        choices=["image", "video-veo", "video-sora", "video-grok", "tts", "history"],
    )
    ...
    try:
        if args.action == "image":
            ...
            result = client.generate_tts(
                args.prompt, model=args.model or "tts-flash", timeout=args.timeout
            )
