...
class ReframeEngine:
    ...
    def simple_center_crop(self, video_path: str, output_path: str = None) -> str:
        """
        Simple center crop to 9:16 (Improved for robustness)
        ...
        # Robust filter: Crop to 9:16 based on input height, then scale to target
        # Filter: crop to target aspect from original, then upscale/downscale
        target_w, target_h = self.get_output_resolution(1080)
        
        # We use a filter complex that works for any input size:
        # 1. Scale to fill height of 1920
        # 2. Crop the center 1080 width
        vf_filter = (
            f"scale=-1:{target_h},"
