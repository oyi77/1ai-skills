    ...
    for day in calendar.get("days", []):
        for post in day.get("posts", []):
            ...
            if platform and media_needed and media_needed != "None":
                ...
                by_platform[platform][media_needed] = (
                    by_platform[platform].get(media_needed, 0) + 1
                )
...
def check_inventory(
    ...
    report = {
        "summary": {
            ...
            "asset_types_missing": len(
                [g for g in gaps.values() if g["status"] == "MISSING"]
            ),
            "asset_types_insufficient": len(
                [g for g in gaps.values() if g["status"] == "INSUFFICIENT"]
            ),
            "coverage_pct": (
                f"{round(len(have)/len(needed)*100, 1)}%" if needed else "100%"
            ),
        },
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Check content inventory against calendar"
    )
    parser.add_argument("calendar_file", help="Calendar JSON file to check against")
    parser.add_argument(
        "--media-dir",
        action="append",
        help="Media directory to scan (repeat for multiple)",
    )
    parser.add_argument("--output", type=str, help="Output report JSON path")
    parser.add_argument(
        "--shoot-list", type=str, help="Output shoot list markdown path"
    )
