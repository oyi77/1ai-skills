...
from seasonal_calendar import (
    get_event_for_date,
    get_seasonal_hashtags,
    get_best_times_override,
    get_seasonal_hook_prefix,
    get_monthly_theme,
)


def generate_caption(
    pillar: str, product: Dict, content_type: str, hook: str, platform: str
) -> str:
...
def generate_calendar(
    ...
    """
    print(
        f"🗓️  Generating {num_days}-day content calendar starting {start_date.isoformat()}..."
    )
    ...
    for i in range(num_days):
        ...
        print(
            f"  ✅ {day_date.strftime('%Y-%m-%d %A')}: {day_calendar['total_posts']} posts"
        )
...
def print_day_summary(day: Dict) -> None:
    ...
    for post in day["posts"]:
        ...
        print(
            f"   {post['time']} {emoji} {post['platform']} | {post['pillar']} | {post['product']}"
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Generate BerkahKarya content calendar"
    )
    parser.add_argument(
        "--start",
        type=str,
    ...
    parser.add_argument(
        "--print", action="store_true", help="Print human-readable output"
    )
