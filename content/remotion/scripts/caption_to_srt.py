...
def json_to_srt(
    ...
    for word in captions:
        ...
        flush = (
            len(buf) >= words_per_line
            or (end - start) >= max_line_ms
            or text.endswith((".", "?", "!", "…"))
        )
    ...
    if buf and start is not None:
        lines.append(
            {"text": " ".join(buf), "startMs": start, "endMs": captions[-1]["endMs"]}
        )
...
def validate_captions(captions: list) -> list[str]:
    ...
    for i, cap in enumerate(captions):
        if cap["startMs"] < prev_end:
            warnings.append(
                f"Word {i} ({cap['text']!r}) overlaps previous: "
...
def main():
    ...
    # ── JSON → SRT ────────────────────────────────────────────
    if (ext == ".json" and not args.reverse) or (
        ext == ".srt" and args.reverse and False
    ):
        ...
        print(
            f"    -vf \"subtitles={out_path}:force_style='FontSize=28,PrimaryColour=&HFFFFFF,Outline=2'\" \\"
        )
