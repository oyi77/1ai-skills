...
Return ONLY valid JSON."""
    ...
    payload = {
        "model": "llama-3.3-70b-versatile" if GROQ_API_KEY else "meta/llama-3.3-70b-instruct",
        "messages": [
...
# ── BYTEPLUS: Generate video (T2V) ──────────────────────────────────────────
def generate_video(prompt: str, ratio: str = "9:16",
                   image_url: Optional[str] = None) -> Optional[str]:
    """
    Generate video via BytePlus Seedance. Returns video URL.
    Uses async task API: create → poll → return URL.
    """
    ...
    try:
        resp = _post(f"{BYTEPLUS_BASE_URL}/contents/generations/tasks", payload, headers)
...
def loop_to_minute(input_path: Path, output_path: Path,
                   target_secs: int = 60, loops: int = 12) -> Optional[Path]:
...
def compress_video(input_path: Path, output_path: Path, crf: int = 28) -> Optional[Path]:
...
class ContentGenerator:
    ...
    async def generate(
        ...
        video_prompt = prompt or content.get("video_prompt") or content.get("prompt", "")
        ...
        print(f"  ✅ Clip: {clip_path} ({clip_size:,}B / {clip_size/1024/1024:.1f}MB)")
...
async def main():
    ...
    parser.add_argument("--concept", default="motivation",
