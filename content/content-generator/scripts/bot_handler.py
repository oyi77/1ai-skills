...
from content_wizard import (
    load_state,
    save_state,
    clear_state,
    build_category_message,
    build_style_message,
    build_format_message,
...
def send_photo(
    chat_id, image_path: str, caption: str = "", buttons: list = None
) -> dict:
...
def send_video(
    chat_id, video_path: str, caption: str = "", buttons: list = None
) -> dict:
...
def run_generation_pipeline(chat_id: str, state: dict):
    ...
    send_photo(
        chat_id,
        img_result,
        f"🖼️ *Preview — {style.replace('_',' ').title()}*\n\n{preview_text}",
        preview_btns,
    )
    ...
    if fmt == "foto":
        ...
        send_text(
            chat_id,
            "✅ *Foto selesai!* Mau diposting ke mana?",
...
def run_i2v_pipeline(chat_id: str, gate: dict):
    ...
    task_id = submit_i2v(
        img_path, config.get("i2v_prompt", f"Animate this {category} product smoothly")
    )
    if not task_id:
        send_text(
            chat_id,
            "❌ Gagal submit I2V task. Coba lagi?",
            [
                [
    ...
    if not vid_result:
        send_text(
            chat_id,
            "❌ Video generation timeout. Coba lagi?",
            [
                [
                    {"text": "🔄 Coba Lagi", "callback_data": "gate:retry"},
                    {"text": "❌ Batal", "callback_data": "gate:cancel"},
...
def handle_callback(cb: dict):
    ...
    if data.startswith("wiz:cat:"):
        ...
        threading.Thread(
            target=run_generation_pipeline, args=(chat_id, state), daemon=True
        ).start()
    ...
    # ── QUALITY GATE callbacks ────────────────────────────────────────
        ...
        threading.Thread(
            target=run_i2v_pipeline, args=(chat_id, gate), daemon=True
        ).start()
        ...
        if gate:
            ...
            threading.Thread(
                target=run_generation_pipeline, args=(chat_id, retry_state), daemon=True
            ).start()
        ...
        clear_state(chat_id)
        edit_message(
            chat_id, msg_id, "❌ Dibatalkan. Kirim foto produk baru untuk mulai lagi."
        )
    ...
    # ── GALLERY callbacks ─────────────────────────────────────────────
        ...
        btns = [
            [
                {
                    "text": f"{'🖼️' if r['type']=='image' else '🎬'} ID {r['id']}",
                    "callback_data": f"gallery:view:{r['id']}",
                }
                for r in results[:3]
            ]
        ]
        if offset >= 5:
            btns.append(
                [
                    {"text": "◀ Prev", "callback_data": f"gallery:list:{offset-5}"},
        ...
        btns = [
            [
                {
                    "text": "📤 Post TikTok",
                    "callback_data": f"gallery:post:{rid}:tiktok",
                },
                {
                    "text": "📤 Post IG",
                    "callback_data": f"gallery:post:{rid}:instagram",
                },
            ],
            [
        ...
        send_text(
            chat_id, f"✅ Ditambahkan ke queue posting ({platform})\nQueue ID: `{qid}`"
        )

    elif data == "gallery:costs":
        text = build_dashboard_message(chat_id)
        send_text(
            chat_id, text, [[{"text": "✖️ Tutup", "callback_data": "gallery:close"}]]
        )
    ...
    # ── BATCH callbacks ───────────────────────────────────────────────
        ...
        cat = data.split(":")[-1]
        save_state(
            chat_id, {**state, "batch_mode": "style_sweep", "batch_category": cat}
        )
        ...
        send_text(
            ...
            [
                [
                    {
                        "text": "🚀 Mulai Batch!",
                        "callback_data": f"batch:run:style:{cat}",
                    },
        ...
        send_text(
            chat_id,
            "Format sweep (Foto, 15s, 30s) — Coming soon! Pakai style sweep dulu ya.",
    ...
    # ── AUTO POST callbacks ───────────────────────────────────────────
        ...
        if not creds.get(platform):
            send_text(
                chat_id,
                f"⚠️ Token untuk {platform} belum dikonfigurasi.\n"
        ...
        edit_message(
            chat_id, msg_id, "✅ Selesai! Konten tersimpan di galeri kamu. /gallery"
        )
            ...
            if r.get("video") and os.path.exists(r["video"]):
                ...
                vid_id = save_result(
                    ...
                    r["video"],
                    {
                        ...
                        "category": category,
                        "style": r["style"],
                        "format": r.get("format", "foto"),
                        "cost_usd": 0.054,
                        "project": os.path.basename(out_dir),
                    },
                )
                send_video(
                    chat_id,
...
def handle_message(msg: dict):
    ...
    if text.startswith("/batch"):
        if state.get("category"):
            ...
            btns = [
                [{"text": v["label"], "callback_data": f"batch:style:{k}"}]
    ...
    if state.get("awaiting") == "edit_caption" and text:
        save_state(
            ...
            {
                **state,
                "pending_caption": {"caption": text, "hashtags": []},
                "awaiting": None,
            },
        )
        send_text(
