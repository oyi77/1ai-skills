...
class FallbackChain:
    ...
    def __init__(self):
        ...
        self._rate_limit_tracker: dict[str, float] = {}  # provider_name → cooldown_until
        self._failure_tracker: dict[str, int] = {}  # provider_name → consecutive failures
        ...
        **kwargs,
        ...
        for provider in chain:
            ...
            if self._is_in_cooldown(provider.provider_name):
                logger.info(
                    "%s in cooldown, skipping", provider.provider_name
                )
                continue
            ...
            try:
                logger.info(
                    ...
                    provider.provider_name, provider_type.value,
                )
                ...
                logger.warning(
                    ...
                    provider.provider_name, attempt.error,
                )

            except RateLimitError as e:
                duration_ms = (time.time() - start) * 1000
                chain_result.attempts.append(ProviderAttempt(
                    provider_name=provider.provider_name,
                    success=False,
        ...
        logger.error(
            ...
            chain_result.total_providers_tried, provider_type.value,
        )
        return chain_result
    ...
    def _set_cooldown(self, provider_name: str, cooldown_seconds: float = 60):
        ...
        logger.info(
            "%s in cooldown for %ds", provider_name, cooldown_seconds
        )
        ...
        for chain_name, chain in [
            ...
            for provider in chain:
                ...
                cooldown_remaining = max(
                    0, self._rate_limit_tracker.get(name, 0) - now
                )
...
def build_default_fallback_chain() -> FallbackChain:
    ...
    chain.register_video_chain([
        BytePlusProvider(),           # 1. Primary - Seedance Pro Fast
        XAIProvider(),                # 2. Tertiary - Grok Aurora
        RemotionVideoProvider(),      # 3. FALLBACK - fully local
        FFmpegSlideshowProvider(),    # 4. LAST RESORT - always works
    ])
