...
def render_animation(animation_config, output_path):
    ...
    cmd = [
        ...
        composition_name,
        "--config",
        str(REMOTION_PROJECT / "remotion.config.js"),
        "--output",
        str(output_path),
    ]
    ...
    try:
        ...
        return {"success": False, "composition": composition_name, "error": str(e)}
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=parallel) as executor:
        ...
        for future in concurrent.futures.as_completed(futures):
            ...
            if result["success"]:
                ...
                print(
                    f"  ❌ {result['composition']} → {result.get('error', 'Unknown error')}"
                )
