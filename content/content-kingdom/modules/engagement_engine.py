...
WAVES = [
    ...
    {"name": "early",   "min_min": 15,  "max_min": 30,  "actions": ["like", "comment"],         "ratio": 0.6},
    {"name": "mid",     "min_min": 60,  "max_min": 120, "actions": ["like", "comment", "share"],"ratio": 0.4},
    {"name": "sustain", "min_min": 180, "max_min": 360, "actions": ["like", "comment"],         "ratio": 0.3},
]

# Authentic-looking comment bank by post type
COMMENT_BANK = {
    "promo":      ["Wah info menarik nih! 👆", "Harga segitu worth it!", "Langsung cek!", "Menarik banget!"],
    "tutorial":   ["Tutorial keren banget!", "Auto saved! 🔖", "Dicoba ah!", "Bermanfaat banget 💡"],
    "motivation": ["Semangat terus! 💪", "Setuju banget ini!", "Relate banget 😭", "Facts!"],
    "product":    ["Wah ini berguna!", "Perlu nih!", "Mantap jiwa 🔥", "Makasih infonya 🙏"],
...
class EngagementEngine(BaseModule):
        ...
        for wave in WAVES:
            ...
            for i, acct in enumerate(wave_accounts):
                ...
                schedule.append({
                    ...
                    "wave":       wave["name"],
                    "trigger_at": (now + timedelta(minutes=base + jitter)).isoformat(),
                    "executed":   False,
                })
        self._append_schedule(schedule)
        print(f"[EngagementEngine] {len(schedule)} actions scheduled for {post_id} across {len(WAVES)} waves")
        return {"post_id": post_id, "total_actions": len(schedule), "schedule": schedule}
    ...
    def schedule_engagement(self, post_id: str, delay_minutes: int) -> Dict:
        ...
        action  = {"post_id": post_id, "action": "like", "wave": "manual",
                   "trigger_at": trigger.isoformat(), "executed": False}
        self._append_schedule([action])
        return action

    def generate_natural_comment(self, post_content: str, persona: str = "default") -> str:
    ...
    def warm_up_account(self, account_id: str) -> Dict:
        ...
        for phase in WARMUP_PHASES:
            for day in range(phase["day_from"], phase["day_to"] + 1):
                for n in range(phase["daily"]):
                    ...
                    hour   = 8 + (n * 50 // 60) % 14
                    minute = (n * 50) % 60
                    run_at = (now + timedelta(days=day)).replace(hour=hour, minute=minute, second=0)
                    plan.append({
        ...
        print(f"[EngagementEngine] Warm-up plan: {len(plan)} actions over 10 days for {account_id}")
...
if __name__ == "__main__":
    ...
    for a in result["schedule"][:4]:
        print(f"  [{a['wave']:8}] {a['account_id']} → {a['action']} @ {a['trigger_at'][11:16]}")
    print(f"  ... +{result['total_actions']-4} more")

    comment = ee.generate_natural_comment("Tutorial cara bikin konten viral pakai AI", "jendralbot_main")
