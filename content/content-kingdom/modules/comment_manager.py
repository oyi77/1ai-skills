...
REPLY_TEMPLATES = {
    "positive": [
        "Makasih banyak! 🙏 {extra}",
        "Terima kasih, semangat terus! 🔥 {extra}",
    ],
    "question": [
        "{answer}\nCek linknya di bio ya! 👆",
        "{answer} — ada pertanyaan lain? DM aja! 💬",
    ],
    "purchase_intent": [
        "Mantap! {prod} Grab di: {url} 🔥",
        "Gas! {prod} Klik sekarang: {url} ⚡",
    ],
    "negative": [
        "Makasih feedbacknya! 🙏 {concern} Boleh DM kalau mau diskusi lebih lanjut."
    ],
}

PRODUCT_KW = {
    "kuliner": "mesin_cetak_kuliner",
    "masak": "mesin_cetak_kuliner",
    "guru": "guru_pintar_ai",
    "belajar": "guru_pintar_ai",
    "konten": "ai_content_pro_seller",
    "content": "ai_content_pro_seller",
    "foto": "sellpix_ai",
    "desain": "sellpix_ai",
    "cashback": "belanja_duit_balik",
    "belanja": "belanja_duit_balik",
    "template": "starter_content_4k",
    "iklan": "ai_creative_ad_engineer",
    "ads": "ai_creative_ad_engineer",
...
class CommentManager(BaseModule):
    ...
    def __init__(self, config: dict):
        ...
        self._pb_url = config.get(
            "postbridge_base_url", "https://api.post-bridge.com/v1"
        )
        self._headers = {
            "Authorization": f"Bearer {_pb}",
    ...
    def classify_comment(self, text: str) -> str:
        ...
        t = text.lower()
        if any(re.search(p, t) for p in SPAM_SIGNALS):
            return "spam"
        if any(re.search(p, t) for p in PURCHASE_SIGNALS):
            return "purchase_intent"
        if any(re.search(p, t) for p in NEGATIVE_SIGNALS):
            return "negative"
    ...
    def generate_reply(
        ...
        if sentiment == "purchase_intent":
            return tmpl.format(
                prod=(
                    f"[{prod['name']} — {prod['price_label']}]"
                    if prod
                    else "[cek semua produk]"
                ),
            ...
            out.append(
                {
                    ...
                    "original": c.get("text"),
                    "dm": (
                        f"Hei {c.get('username','kak')}! 👋 Info lengkap "
    ...
    def _pick_product(
        self, text: str, override: Optional[str] = None
    ) -> Optional[Dict]:
    ...
    tests = [
        {
            ...
            "user_id": "u1",
            "username": "budi",
            "text": "Pengen beli dong, link mana kak?",
        },
        {"id": "2", "user_id": "u2", "username": "sari", "text": "Berapa harganya?"},
        {
            ...
            "user_id": "u3",
            "username": "andi",
            "text": "Keren banget kontennya! 🔥",
        },
        {
            "id": "4",
            "user_id": "u4",
            "username": "spam",
            "text": "f4f cek ig saya free followers",
        },
        {
            "id": "5",
            "user_id": "u5",
            "username": "dewi",
            "text": "Bisa dipake di HP gak?",
        },
        {
            "id": "6",
            "user_id": "u6",
            "username": "rizky",
            "text": "Harga segini mahal, gak worth",
        },
    ]
