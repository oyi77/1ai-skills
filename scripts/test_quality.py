...
FM_KEY_VALUE_RE = re.compile(r"^([a-zA-Z_][\w-]*):\s*(.*)")
SECTION_HDR_RE = re.compile(r"^##\s+(.+?)\s*$")
SECTION_HDR_MULTI_RE = re.compile(r"^##\s+(.+?)\s*$", re.MULTILINE)
ANY_HDR_RE = re.compile(r"^#{1,6}\s+")
TODO_CHECKLIST_RE = re.compile(r"^\s*-\s*\[")
...
def find_sections(body: str) -> list[str]:
    ...
    sections = []
    for m in SECTION_HDR_MULTI_RE.finditer(body):
...
def validate_skill(path: str) -> dict:
    ...
    todo_lines = [
        ...
        if "[TODO]" in line and not TODO_CHECKLIST_RE.match(line)
    ]
    if todo_lines:
