...
FIND_SECTIONS_RE = re.compile(r"^##\s+(.+?)\s*$", re.MULTILINE)
HDR_RE = re.compile(r"^##\s+(.+?)\s*$")
SUB_HDR_RE = re.compile(r"^#{1,6}\s+")
TODO_IGNORE_RE = re.compile(r"^\s*-\s*\[")
...
def find_sections(body: str) -> list[str]:
    ...
    sections = []
    for m in FIND_SECTIONS_RE.finditer(body):
...
def validate_skill(path: str) -> dict:
    ...
    todo_lines = [
        ...
        if "[TODO]" in line and not TODO_IGNORE_RE.match(line)
    ]
    if todo_lines:
