...
_HEADER_MULTILINE_RE = re.compile(r"^##\s+(.+?)\s*$", re.MULTILINE)
_HEADER_EXACT_RE = re.compile(r"^##\s+(.+?)\s*$")
_HEADER_ANY_RE = re.compile(r"^#{1,6}\s+")
_TODO_CHECKLIST_RE = re.compile(r"^\s*-\s*\[")
...
def find_sections(body: str) -> list[str]:
    ...
    sections = []
    for m in _HEADER_MULTILINE_RE.finditer(body):
...
def find_empty_sections(body: str) -> list[str]:
    ...
    for i, line in enumerate(lines):
        hdr = _HEADER_EXACT_RE.match(line)
        if not hdr:
            continue
        # Look at next non-empty line
        for j in range(i + 1, len(lines)):
            if lines[j].strip():
                if _HEADER_ANY_RE.match(lines[j]):
...
def validate_skill(path: str) -> dict:
    ...
    if "[TODO]" in text:
        todo_lines = [
            ...
            if "[TODO]" in line and not _TODO_CHECKLIST_RE.match(line)
        ]
        if todo_lines:
