...
async def social_spy(target: str) -> dict:
    ...
    try:
        r = subprocess.run(
            ["twitter", "search", clean_target, "-n", "20", "--yaml"],
            capture_output=True,
            text=True,
            env=env,
            timeout=20,
        ...
        result["platforms"]["twitter"] = {
            ...
            "engagement_rate": (
                f"{(total_likes/total_views*100):.1f}%" if total_views > 0 else "N/A"
            ),
        ...
        for t in texts[:10]:
            if any(
                w in t.lower()
                for w in ["cara", "teman", "stop", "90%", "gratis", "tanpa"]
            ):
                hooks.append(t[:100])
    ...
    try:
        ...
        likes_match = _re.search(r'"heartCount":(\d+)', html) or _re.search(
            r'"heart":(\d+)', html
        )
        ...
        video_items = _re.findall(
            r'"id":"(\d+)".*?"desc":"([^"]*)".*?"playCount":(\d+).*?"diggCount":(\d+)',
            html,
        )
        top_videos = []
    ...
    try:
        ...
        sub_match = re.search(
            r'"subscriberCountText":\{"simpleText":"([\d.]+[KMB]?) subscriber', yt_html
        )
        vid_count_match = re.search(
            r'"videosCountText":\{"runs":\[\{"text":"([\d,]+)"', yt_html
        )
    ...
    try:
        ...
        if tweet_dates:
            ...
            if parsed:
                ...
                posts_per_week = (
                    round(len(parsed) / (date_range / 7), 1)
                    if date_range >= 7
                    else len(parsed)
                )
                ...
                result["content_strategy"]["posting_schedule"] = {
                    ...
                    "best_day_of_week": (
                        day_counts.most_common(1)[0][0] if day_counts else "unknown"
                    ),
                    "best_hour_of_day": (
                        hour_counts.most_common(1)[0][0] if hour_counts else "unknown"
                    ),
                        ...
                        result["products"].append(
                            {
                                ...
                                "rating": (
                                    float(rating_obj.get("ratingValue", 0))
                                    if rating_obj
                                    else None
                                ),
                                "reviews": (
                                    int(rating_obj.get("reviewCount", 0))
                                    if rating_obj
                                    else 0
                                ),
                            }
...
async def lynk_spy(username: str) -> dict:
    ...
    try:
        ...
        product_names = re.findall(
            r'class="[^"]*product[^"]*name[^"]*"[^>]*>([^<]+)', html
        )
        if not product_names:
            product_names = re.findall(r"<h\d[^>]*>([^<]{3,50})</h\d>", html)

        prices = re.findall(r"(?:Rp|IDR)[.\s]?([\d.,]+)", html)
        original_prices = re.findall(
            r"(?:line-through|strikethrough)[^>]*>(?:Rp|IDR)[.\s]?([\d.,]+)", html
        )
        ...
        for i, name in enumerate(product_names[:30]):
            product = {
                ...
                "original_price": (
                    f"Rp {original_prices[i]}" if i < len(original_prices) else None
                ),
        ...
        if result["products"] and prices:
            try:
                ...
                if price_values:
                    ...
                    est_monthly = (
                        result["total_products"] * avg_price * 15
                    )  # 15 sales/month/product conservative
...
def content_calendar_spy(twitter_data: dict) -> dict:
    ...
    for day in day_order:
        ...
        for h in range(0, 24, 3):
            count = sum(
                hour_dist.get(str(hh), hour_dist.get(hh, 0)) for hh in range(h, h + 3)
            )
...
async def revenue_spy(
    target: str,
    social_data: dict = None,
    funnel_data: dict = None,
    lynk_data: dict = None,
    gumroad_data: dict = None,
) -> dict:
    ...
    if social_data and social_data.get("platforms"):
        ...
        if avg_views > 0:
            ...
            try:
                avg_price = sum(
                    float(p.replace(".", "").replace(",", "")) for p in prices[:3]
                ) / len(prices[:3])
            except:
                avg_price = 75000
    ...
    if lynk_data and lynk_data.get("products"):
        ...
        lynk_monthly = (
            product_count * avg_price * est_conversion * 1000
        )  # estimated visitors
...
async def tech_spy(target: str) -> dict:
    ...
    try:
        ...
        result["headers"] = {
            ...
            if k.lower()
            in ["server", "x-powered-by", "content-type", "cf-ray", "x-vercel-id"]
        }
        ...
        if "gtag" in html or "G-" in html:
            ...
            result["analytics"].append(
                f'Google Analytics {ga_id.group(0) if ga_id else ""}'
            )
    ...
    try:
        ...
        whois_r = subprocess.run(
            ["whois", domain], capture_output=True, text=True, timeout=5
        )
...
def generate_report(target: str, all_data: dict, mode: str = "report") -> str:
    ...
    if "tech" in all_data:
        ...
        if t.get("whois"):
            w = t["whois"]
            lines.append(
                f'- Domain created: **{w.get("created")}** via {w.get("registrar")}'
            )
        lines.append("")
    ...
    if "model" in all_data:
        ...
        for k, v in m.get("clone_effort", {}).items():
            if k != "total_estimate":
                lines.append(f"- {k}: **{v}**")
        lines.append(
            f'- **TOTAL: {m.get("clone_effort", {}).get("total_estimate", "unknown")}**'
        )
        lines.append("")
    ...
    if "gumroad" in all_data:
        ...
        for p in g.get("products", [])[:5]:
            lines.append(
                f'  - {p["name"]}: {p["price"]} ({p.get("reviews", 0)} reviews)'
            )
        lines.append("")
    ...
    if "lynk" in all_data:
        ...
        if l.get("social_links"):
            lines.append(
                f'- Social links: {", ".join(s["platform"] for s in l["social_links"])}'
            )
        lines.append("")
    ...
    if "landing_page" in all_data:
        ...
        if trust:
            lines.append(
                f'- Testimonials: **{trust.get("testimonials_count", 0)}** | Reviews: **{trust.get("review_count", 0)}** | Badges: {", ".join(trust.get("badges", []))}'
            )
        urgency = lp.get("urgency", {})
        if urgency.get("scarcity_text") or urgency.get("fomo_triggers"):
            lines.append(
                f'- Urgency: Scarcity={urgency.get("scarcity_text", [])}, FOMO={urgency.get("fomo_triggers", [])}'
            )
        ...
        if bm.get("pricing_tiers"):
            ...
            for tier in bm["pricing_tiers"][:5]:
                if isinstance(tier, dict):
                    lines.append(
                        f'  - {tier.get("name", "?")} : {tier.get("price", "?")} / {tier.get("period", "?")}'
                    )
        lines.append("")
    ...
    if "content_intel" in all_data:
        ...
        if cs.get("hashtags_used"):
            lines.append(
                f'- Top Hashtags: {", ".join("#" + h for h in cs["hashtags_used"][:10])}'
            )
        # Per-platform summary
        tt = ci.get("tiktok", {})
        if tt and not tt.get("error"):
            lines.append(
                f'- TikTok: **{tt.get("followers", 0):,}** followers, **{tt.get("video_count", 0)}** videos, ER={tt.get("engagement_rate", "N/A")}'
            )
        ig = ci.get("instagram", {})
        if ig and not ig.get("error"):
            lines.append(
                f'- Instagram: **{ig.get("followers", 0):,}** followers, **{ig.get("posts_count", 0)}** posts'
            )
        yt = ci.get("youtube", {})
        if yt and not yt.get("error"):
            lines.append(
                f'- YouTube: **{yt.get("subscribers", "?")}** subscribers, **{yt.get("total_videos", "?")}** videos'
            )
        lines.append("")
    ...
    if "revenue_intel" in all_data:
        ...
        if ri.get("revenue_sources"):
            ...
            for src in ri["revenue_sources"][:5]:
                lines.append(
                    f'  - {src.get("source", "?")}: IDR {src.get("estimated_monthly_idr", 0):,}/mo [{src.get("confidence", "?")}]'
                )
        ue = ri.get("unit_economics", {})
        if ue.get("cac_estimate_idr"):
            lines.append(
                f'- Unit Economics: CAC=IDR {ue["cac_estimate_idr"]:,} | LTV=IDR {ue.get("ltv_estimate_idr", 0):,} | Ratio={ue.get("ltv_cac_ratio", 0)}'
            )
    ...
    if "market_opportunity" in all_data:
        ...
        if opps:
            ...
            for i, opp in enumerate(opps, 1):
                ...
                lines.append(
                    f'   - Effort: {opp.get("effort", "?")} | Time: {opp.get("time_to_revenue", "?")} | Revenue: IDR {opp.get("revenue_potential_idr", 0):,}/mo'
                )
...
async def ads_intelligence_spy(target_name: str, page_id: str = None) -> dict:
    ...
    try:
            ...
            try:
                ...
                with sync_playwright() as p:
                    ...
                    ad_cards = page.query_selector_all(
                        '[class*="xrvj5dj"], [class*="ad-card"], [data-testid*="ad"]'
                    )
                    if not ad_cards:
                    ...
                    for i, card in enumerate(ad_cards[:20]):
                        try:
                            ...
                            date_match = _re.search(
                                r"Started running on\s*(\w+ \d+,?\s*\d{4})", text
                            )
        ...
        if fb_data_holder[0]:
            ...
            if fb["ads"]:
                avg_days = sum(a.get("days_running", 30) for a in fb["ads"]) / len(
                    fb["ads"]
                )
            else:
                avg_days = 30
            result["fb_ads"]["estimated_spend_idr"] = int(
                fb["count"] * 300000 * avg_days / 30
            )
        ...
        def run_tt():
            try:
                ...
                with sync_playwright() as p:
                    ...
                    hooks = []
                    ad_texts = page.query_selector_all(
    ...
    try:
        all_ctas = [
            a.get("cta", "") for a in result["fb_ads"].get("ads", []) if a.get("cta")
        ]
        all_hooks = [
            a.get("headline", "")
            for a in result["fb_ads"].get("ads", [])
            if a.get("headline")
        ]
        ...
        result["winning_patterns"]["top_ctas"] = [
            c for c, _ in Counter(all_ctas).most_common(5)
        ]
...
async def landing_page_deep_spy(url: str) -> dict:
    ...
    try:
        ...
        if review_match:
            result["trust_signals"]["review_count"] = int(
        ...
        result["trust_signals"]["badges"] = [
            b for b in badge_keywords if b in body_lower
        ]

        # Urgency signals
        result["urgency"]["countdown_timer"] = bool(
            _re.search(r"countdown|timer|hour.*minute.*second", html.lower())
        )
        scarcity_keywords = [
            "terbatas",
            "limited",
        ...
        result["urgency"]["scarcity_text"] = [
            s for s in scarcity_keywords if s in body_lower
        ]
        fomo_keywords = [
        ...
        result["urgency"]["fomo_triggers"] = [
            f for f in fomo_keywords if f in body_lower
        ]
        ...
        if _re.search(r"(?:Rp|IDR|USD|\$)\s*[\d.,]+", body_text):
            if _re.search(
                r"(?:per\s*(?:bulan|month|tahun|year))|(?:/(?:mo|month|yr|year|bulan))",
                body_lower,
            ):
        ...
        sp_matches = _re.findall(
            r"(\d[\d,.]*)\+?\s*(?:pengguna|user|customer|pelanggan|orang|member|download|subscriber)",
            body_lower,
        )
        ...
        if result["lead_magnets"]:
            ...
            kw in body_lower for kw in ["demo", "jadwalkan", "schedule", "book a call"]
        ):
            result["funnel_guess"] = "demo_call"
        elif result["price_display_type"] == "free" or "freemium" in body_lower:
            result["funnel_guess"] = "freemium"
        elif any(
            kw in body_lower for kw in ["affiliate", "afiliasi", "referral", "komisi"]
        ):
...
async def content_intelligence_spy(handles: dict) -> dict:
    ...
    if tiktok_handle:
        try:
            ...
            def run_tiktok():
                try:
                    with sync_playwright() as p:
                        ...
                        page = context.new_page()
                        page.goto(
                            f"https://www.tiktok.com/@{tiktok_handle}",
                            wait_until="domcontentloaded",
                            timeout=15000,
                        )
                        page.wait_for_timeout(4000)
            ...
            if html:
                ...
                likes = _re.search(r'"heartCount":(\d+)', html) or _re.search(
                    r'"heart":(\d+)', html
                )
                ...
                if not video_items:
                    ...
                    video_items = [
                        (v[0], v[1], v[2], v[3], "0", "0") for v in video_items
                    ]
                ...
                if result["tiktok"].get("video_count", 0) > 0:
                    result["tiktok"]["posting_frequency"] = (
                        "active" if result["tiktok"]["video_count"] > 50 else "moderate"
                    )
                ...
                if top_videos:
                    ...
                    result["tiktok"][
                        "engagement_rate"
                    ] = f'{(sum(v["likes"] for v in top_videos) / len(top_videos) / total_followers * 100):.2f}%'
    ...
    if ig_handle:
        try:
            ...
            html = ig_holder[0] or ""
            if html:
                followers = _re.search(
                    r'"edge_followed_by":\{"count":(\d+)', html
                ) or _re.search(r'"follower_count":(\d+)', html)
                following = _re.search(
                    r'"edge_follow":\{"count":(\d+)', html
                ) or _re.search(r'"following_count":(\d+)', html)
                posts = _re.search(
                    r'"edge_owner_to_timeline_media":\{"count":(\d+)', html
                ) or _re.search(r'"media_count":(\d+)', html)
                ...
                # Story highlights
                highlight_count = len(
                    _re.findall(r'"highlight_reel_count":(\d+)', html)
                )
    ...
    if yt_handle:
        try:
            ...
            sub_match = _re.search(
                r'"subscriberCountText":\{"simpleText":"([\d.]+[KMB]?) subscriber', html
            )
            vid_count = _re.search(
                r'"videosCountText":\{"runs":\[\{"text":"([\d,]+)"', html
            )
    ...
    if tw_handle:
        result["twitter"] = {
            "handle": tw_handle,
            "note": "Use social_spy for detailed Twitter data",
        }
    ...
    try:
        ...
        if platform_followers:
            result["content_strategy"]["primary_platform"] = max(
                platform_followers, key=platform_followers.get
            )
            result["content_strategy"]["engagement_rate"] = {}
            if "tiktok" in result and result["tiktok"].get("engagement_rate"):
                result["content_strategy"]["engagement_rate"]["tiktok"] = result[
                    "tiktok"
                ]["engagement_rate"]
        ...
        all_descs = " ".join(
            v.get("desc", "") for v in result.get("tiktok", {}).get("top_videos", [])
        )
        hashtags = _re.findall(r"#(\w+)", all_descs)
        if hashtags:
            from collections import Counter

            result["content_strategy"]["hashtags_used"] = [
                h for h, _ in Counter(hashtags).most_common(10)
            ]
...
def promotion_strategy_spy(target: str, all_data: dict) -> dict:
    ...
    try:
        ...
        for plat in ["tiktok", "instagram", "youtube"]:
            ...
            if (
                ci_data
                and not ci_data.get("error")
                and ci_data.get("followers", ci_data.get("subscribers", 0))
            ):
        ...
        if fb_ad_count > 5 or has_pixels:
            result["paid_vs_organic"] = (
                "mostly_paid" if fb_ad_count > 10 else "balanced"
            )
        else:
        ...
        result["has_affiliate_program"] = any(
            kw in combined_text
            for kw in ["affiliate", "afiliasi", "referral", "komisi"]
        )

        # Telegram community
        result["has_telegram_community"] = any(
            kw in combined_text for kw in ["t.me/", "telegram", "join group"]
        )

        # Email list
        result["has_email_list"] = any(
            kw in combined_text
            for kw in ["newsletter", "email list", "subscribe", "berlangganan"]
        )
        ...
        result["influencer_marketing"] = any(
            kw in combined_text
            for kw in ["collab", "kolaborasi", "influencer", "kol", "endorsement"]
        )
...
def revenue_intelligence(all_data: dict, target_name: str) -> dict:
    ...
    try:
        ...
        if isinstance(ads, dict):
            fb_ads = ads.get("fb_ads", ads.get("facebook_ads", {}))
        fb_count = (
            fb_ads.get("count", fb_ads.get("active_ads_found", 0))
            if isinstance(fb_ads, dict)
            else 0
        )
        ...
        if bot and isinstance(bot, dict):
            ...
            tt_data = content_intel.get("tiktok", {}) or social.get(
                "platforms", {}
            ).get("tiktok", {})
        ...
        sorted_sources = sorted(
            result["revenue_sources"],
            key=lambda x: x.get("estimated_monthly_idr", 0),
            reverse=True,
        )
        ...
        if (
            result["paid_vs_organic"]
            if "paid_vs_organic" in result
            else all_data.get("promotion", {}).get("paid_vs_organic") == "mostly_paid"
        ):
    ...
    try:
        ...
        resp = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are a business strategist for an Indonesian digital product company. Return only valid JSON arrays.",
                },
...
async def run_spy(
    target: str, modules: list, mode: str = "report", url: str = None
) -> dict:
    ...
    if "social" in modules or "content" in modules:
        ...
        tw = s.get("platforms", {}).get("twitter", {})
        print(
            f'   Twitter: {tw.get("posts_found",0)} posts | {tw.get("avg_views_per_post",0):,} avg views'
        )
    ...
    if "ads" in modules:
        ...
        print(
            f'   FB ads: {all_data["ads"]["facebook_ads"].get("active_ads_found", "?")}'
        )
    ...
    if "tech" in modules:
        ...
        print(
            f'   Stack: {all_data["tech"].get("framework")} / {all_data["tech"].get("hosting")}'
        )
    ...
    if "gumroad" in modules:
        ...
        g = all_data["gumroad"]
        print(
            f'   Products: {g.get("total_products", 0)} | Rev: {g.get("estimated_monthly_revenue", "?")}'
        )
    ...
    if "lynk" in modules:
        ...
        l = all_data["lynk"]
        print(
            f'   Products: {l.get("total_products", 0)} | Rev: {l.get("estimated_revenue", "?")}'
        )
    ...
    if "content-calendar" in modules or "content" in modules:
        if "social" in all_data:
            ...
            cc = all_data["content_calendar"]
            print(
                f'   Consistency: {cc.get("consistency_score", 0)}/10 | Best days: {cc.get("best_days", [])}'
            )
    ...
    if all_data["competitive_opportunities"]:
        print(
            f'Target: {len(all_data["competitive_opportunities"])} opportunities found'
        )
    ...
    if "landing_page" in modules:
        ...
        try:
            ...
            lp = all_data["landing_page"]
            print(
                f'   H1: {lp.get("h1", "?")[:60]} | Funnel: {lp.get("funnel_guess", "?")} | Load: {lp.get("page_load_time_ms", 0)}ms'
            )
        except Exception as e:
    ...
    if "business_model" in modules:
        ...
        try:
            ...
            bm = all_data["business_model"]
            print(
                f'   Type: {bm.get("business_model_type", "?")} | Score: {bm.get("revenue_model_score", 0)}/10'
            )
        except Exception as e:
    ...
    if "ads_intel" in modules:
        ...
        try:
            ...
            ai = all_data["ads_intel"]
            print(
                f'   FB ads: {ai.get("fb_ads", {}).get("count", 0)} | Budget: {ai.get("budget_estimate", "?")}'
            )
        except Exception as e:
    ...
    if "content_intel" in modules:
        ...
        try:
            ...
            cs = ci.get("content_strategy", {})
            print(
                f'   Primary: {cs.get("primary_platform", "?")} | Hooks: {len(cs.get("viral_hooks", []))}'
            )
        except Exception as e:
    ...
    if "promotion" in modules:
        ...
        try:
            ...
            ps = all_data["promotion"]
            print(
                f'   Strategy: {ps.get("growth_strategy", "?")} | Channels: {", ".join(ps.get("active_channels", []))}'
            )
        except Exception as e:
    ...
    if "revenue_intel" in modules:
        ...
        try:
            ...
            est = ri.get("total_revenue_estimate", {})
            print(
                f'   Estimate: {est.get("low", "?")} - {est.get("high", "?")} | Score: {ri.get("revenue_model_score", 0)}/10'
            )
        except Exception as e:
    ...
    if "market_opportunity" in modules:
        ...
        try:
            ...
            for i, opp in enumerate(opps[:3], 1):
                print(
                    f'   {i}. {opp.get("opportunity", "?")[:60]} (IDR {opp.get("revenue_potential_idr", 0):,}/mo)'
                )
        except Exception as e:
...
async def main():
    ...
    parser.add_argument(
        "--target", required=True, help="Target: URL, @bot, or business name"
    )
    parser.add_argument(
        "--modules",
        default="social,tech,funnel,revenue,model",
        help="Comma-separated modules",
    )
    parser.add_argument("--all", action="store_true", help="Run all modules")
    parser.add_argument(
        "--mode", default="report", choices=["report", "clone", "improve"]
    )
    ...
    # Run
    all_data = await run_spy(
        args.target, modules, args.mode, url=getattr(args, "url", None)
    )
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(
            {"target": args.target, "data": all_data, "report": report},
            f,
            indent=2,
            ensure_ascii=False,
            default=str,
