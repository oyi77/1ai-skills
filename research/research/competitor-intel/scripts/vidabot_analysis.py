...
sys.path.insert(0, '/mnt/data/berkahkarya/skills/1ai-skills/development/bot-extractor/scripts')
...
def main():
    ...
    for key, analysis in arch['format_analysis'].items():
        print(f'      {key[:50]}: {analysis["format_type"]} | {analysis["length"]} chars | emojis: {analysis["emojis_used"]}')
    print()
    ...
    print(f'      Language ratio: ID={bp["language_ratio"]["indonesian"]}, EN={bp["language_ratio"]["english"]}')
    print(f'      Formality: {bp["formality"]}')
    print(f'      Tone: {bp["tone"]}')
    print(f'      Emoji density: {bp["emoji_density"]} ({bp["emoji_per_message"]} per msg)')
    ...
    print(f'      Estimated hours: {cd["estimated_hours"]}h ({cd["estimated_days"]} days)')
