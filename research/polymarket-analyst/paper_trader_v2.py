...
class PaperTrader:
    ...
    def place_trade(self, market_id: str, market_title: str, side: str, 
                    amount: float, entry_odds: float, strategy: str) -> Dict:
        """Place a paper trade"""
        
        available = self.get_available_balance()
        if amount > available:
            return {'error': f'Insufficient balance. Available: ${available:.2f}, Requested: ${amount:.2f}'}
        ...
        for m in markets:
            ...
            if 0.3 < yes_odds < 0.7:
                opportunities.append({
                    ...
                    'yes_odds': yes_odds,
                    'no_odds': 1 - yes_odds,
                    'category': m.get('tags', ['Unknown'])[0] if m.get('tags') else 'Unknown'
                })
    ...
    def auto_trade_strategy(self):
        ...
        if available < 10:
            return {'status': 'INSUFFICIENT_BALANCE', 'message': f'Only ${available:.2f} available'}
    ...
    def print_report(self):
        ...
        print(f"Return:               {(r['pnl']['total']/r['balance']['initial']*100):+.2f}%")
        print()
        ...
        if r['open_positions']:
            for i, pos in enumerate(r['open_positions'], 1):
                ...
                print(f"   Side: {pos['side']} | Amount: ${pos['amount']:.2f} | Entry: {pos['entry_odds']*100:.1f}%")
        ...
        for log in r['decision_log']:
            ...
            if log['action'] == 'PLACE_TRADE':
                print(f"   {log['side']} ${log['amount']} on '{log['market']}' @{log['odds']*100:.1f}%")
