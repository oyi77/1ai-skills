...
class TaskRunner:
    ...
    def submit(self, serial: str, task_type: str,
    ...
    def submit_all(self, serials: list[str], task_type: str,
                   priority: int = PRIORITY_NORMAL,
    ...
    def _worker(self):
        while self._running:
            try:
                ...
                if device and not device.get("connected", 0) and device.get("error_count", 0) > 5:
                    log.debug(f"Skipping task {task.task_type} — device {task.serial} dead")
    ...
    def _execute_task(self, task: Task):
        try:
            ...
            log.warning(f"Task {task.task_type} on {task.serial} failed (attempt {task.attempt+1}): {e}")
    ...
    def _execute(self, serial: str, task_type: str, params: dict) -> dict:
        ...
        try:
            ...
            db.insert_task(
                serial=serial, device_name=name, task_type=task_type,
                success=success, data=data, error=error,
                duration_ms=duration_ms, screenshot=screenshot,
            )
        return data
    ...
    def _task_health_check(self, serial: str, params: dict) -> dict:
        ...
        if state.battery >= 0 and state.battery <= 10:
            ...
            if not db.is_alert_recent(serial, "battery_critical"):
                db.insert_alert(serial, "battery_critical",
            ...
            if not db.is_alert_recent(serial, "battery_low", 3600):
                db.insert_alert(serial, "battery_low",
                                f"🟡 {device.get('name',serial)} battery: {state.battery}%")
        log.info(f"Health [{device.get('name',serial)}]: battery={state.battery}% "
    ...
    def _task_app_check(self, serial: str, params: dict) -> dict:
        ...
        for skill, pkg in apps.items():
            app_status[skill] = {"package": pkg, "running": self.dm.is_app_running(serial, pkg)}
    ...
    def _task_tiktok_inbox(self, serial: str, params: dict) -> dict:
        ...
        return {"status": "checked", "screenshot": ss, "screenshot_inbox": ss2, "inbox_tapped": found}
    ...
    def _task_instagram_dms(self, serial: str, params: dict) -> dict:
        ...
        time.sleep(2)
        ss2 = self.dm.screenshot(serial)
        return {"status": "checked", "screenshot": ss, "screenshot_dm": ss2, "dm_tapped": found}
    ...
    def get_recent_results(self, serial: str = None, limit: int = 20) -> list[dict]:
