...
SELECTORS_FILE = Path("/home/openclaw/.openclaw/workspace/skills/tiktok-automation/selectors.json")
...
async def login_single_account(account: dict, playwright_context):
    ...
    try:
        ...
        if 'otp' in current_url.lower() or 'verification' in current_url.lower():
            need_otp = True
        elif 'two-factor' in current_content.lower() or '2fa' in current_content.lower():
            need_otp = True
        
        # Check Email indicators
        if 'email' in current_url.lower() and 'verify' in current_url.lower():
            need_email = True
        elif 'check your email' in current_content.lower() or 'email code' in current_content.lower():
            need_email = True
        ...
        if need_otp:
            ...
            # Submit OTP
            otp_field = await page.wait_for_selector('input[placeholder*="code"]', timeout=10000)
            if otp_field:
                await otp_field.type(otp_input)
                submit_otp = await page.wait_for_selector('button[type="submit"]', timeout=10000)
                if submit_otp:
        ...
        if 'login' not in final_url.lower():
            ...
            session_file = Path(f"/home/openclaw/.openclaw/workspace/skills/tiktok-automation/sessions/{username}_session.json")
...
async def multi_account_login():
    ...
    async with async_playwright() as playwright_context:
        ...
        print(f"   /home/openclaw/.openclaw/workspace/skills/tiktok-automation/sessions/")
