...
# Asset paths
HOOK_FRAMES_DIR = Path(
    "/home/openclaw/.openclaw/workspace/skills/1ai-skills/tiktok-automation/assets"
)
...
def upload_media_to_postbridge(filepath):
    ...
    payload = {"mime_type": "image/png", "size_bytes": file_size, "name": filename}
    ...
    upload_response = requests.put(
        upload_url, headers={"Content-Type": "image/png"}, data=binary_data, timeout=60
    )
...
def post_to_social_accounts(media_id, caption, hashtag_str):
    ...
    response = requests.post(
        f"{POSTBRIDGE_BASE_URL}/v1/posts", headers=headers, json=payload, timeout=30
    )
...
def upload_hook_frame(
    product_key, hook_type, caption, hashtag_str, affiliate_link, filepath
):
...
def batch_upload():
    ...
    print(
        f"📱 Social Accounts: {len(SOCIAL_ACCOUNTS)} (YouTube, Instagram, Twitter, Facebook)"
    )
    print("=" * 80 + "\n")
...
def main():
    ...
    print(
        "🚀 Uploading 18 hook frames ke social media (YouTube, Instagram, Twitter, Facebook)"
    )
