...
def check_postbridge_accounts():
    ...
    try:
        ...
        response = requests.get(
            f"{POSTBRIDGE_BASE_URL}/accounts",
            headers=headers,
            timeout=30
        )
        ...
        if response.status_code == 200:
            ...
            for acc in accounts.get('data', []):
                print(f"   - Platform: {acc.get('platform', 'N/A')}, Account: {acc.get('username', 'N/A')}")
...
def upload_to_postbridge_v2(product_key, hook_type, caption, hashtags, affiliate_link, filepath, social_account_id=None):
