#!/usr/bin/env python3
"""
JENDRALBOT PostBridge TikTok Uploader - REPAIRED VERSION
Upload 18 hook frames ke TikTok using PostBridge API dengan structure yang benar
"""
...
def check_postbridge_accounts():
    """Check social accounts yang connected di PostBridge"""
    try:
        headers = {"Authorization": f"Bearer {POSTBRIDGE_API_KEY}"}

        response = requests.get(
            f"{POSTBRIDGE_BASE_URL}/accounts", headers=headers, timeout=30
        )

        if response.status_code == 200:
            accounts = response.json()
            print(f"\n✅ Connected Accounts:")
            print(f"   Total: {len(accounts.get('data', []))} accounts")
            for acc in accounts.get("data", []):
                print(
                    f"   - Platform: {acc.get('platform', 'N/A')}, Account: {acc.get('username', 'N/A')}"
                )
            return accounts.get("data", [])
        else:
            print(f"\n⚠️ Error checking accounts: Status {response.status_code}")
...
def upload_to_postbridge_v2(
    ...
    """Upload single hook frame ke PostBridge TikTok API dengan structure yang benar"""
    ...
    print(f"   Social Account: {social_account_id}")

    # Prepare payload sesuai dokumentasi PostBridge
    payload = {"platform": "tiktok", "content": full_caption, "platforms": ["tiktok"]}

    # Jika social account ID disediakan
    ...
    try:
        ...
        with open(filepath, "rb") as f:
            files = {"media": (filepath.name, f, "image/png")}

            # Send dengan multipart/form-data
...
def batch_upload_via_postbridge_v2():
    """Upload semua hook frames ke TikTok menggunakan PostBridge API dengan structure yang benar"""

    file_mapping = get_file_mapping()
    social_accounts = check_postbridge_accounts()

    print("\n" + "=" * 80)
    print("🚀 JENDRALBOT - POSTBRIDGE TIKTOK UPLOAD (V2 - REPAIRED)")
    print("=" * 80)
    ...
    if not social_accounts:
        print("\n⚠️ WARNING: No social accounts connected to PostBridge!")
        print("   Boss, login ke PostBridge dashboard dulu:")
        print("   https://dashboard.post-bridge.com/accounts")
        print("   Connect TikTok account yang mau dipakai untuk posting")
        print("   Kalau sudah, jalankan script lagi!\n")
        return
    ...
    for filename, info in file_mapping.items():
        ...
        # Upload via PostBridge V2
        result = upload_to_postbridge_v2(
            ...
            filepath=filepath,
            social_account_id=social_accounts[0].get("id") if social_accounts else None,
        )
    ...
    print("=" * 80)
    print(f"✅ Successful uploads: {success_count}/{len(_results)}")
    print(f"❌ Failed uploads: {len(_results) - success_count}/{len(_results)}")

    if len(_results) - success_count > 0:
        print(f"\n⚠️ Failed uploads:")
        for result in _results:
            if not result["success"]:
                print(
                    f"   - {Path(result['file']).name}: {result.get('error', 'Unknown')}"
                )

    print("\n✅ POSTBRIDGE UPLOAD COMPLETE!")
    if success_count > 0:
        print(f"📱 {success_count} hook frames berhasil di-upload ke TikTok")
        print(f"💰 Siap untuk TRACKING: https://lynk.id/jendralbot")
    print("")


def main():
    print("🎯 POSTBRIDGE TIKTOK UPLOAD - JENDRALBOT CAMPAIGN (REPAIRED)")
    print("🚀 Uploading 18 hook frames ke TikTok using API with correct structure")
