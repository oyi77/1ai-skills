...
## 2025-08-01 - [Cache Expensive System Checks in Large Iterations]
**Learning:** In the comprehensive `test-skills.py` test suite, repeatedly evaluating dynamic inputs via `__import__()` and `ast.parse()` across thousands of files generated massive redundant performance overhead due to the high frequency of `ImportError` and `SyntaxError` exception handling and parsing times.
**Action:** Always employ module-level memoization dictionaries (like `_IMPORT_CACHE` and `_AST_CACHE`) when repeating slow native evaluations inside global loops to dramatically lower CPU usage and execution time.
