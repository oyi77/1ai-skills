...
## 2025-07-26 - [Optimize YAML Serialization Speed]
**Learning:** Serializing hundreds or thousands of YAML files (e.g., `SKILL.md` frontmatter) using `yaml.safe_dump()` in pure Python creates a CPU bottleneck during batch operations, similar to `yaml.safe_load()`.
**Action:** When working with PyYAML for large-scale file writing or frontmatter modifications, switch to `yaml.dump(..., Dumper=getattr(yaml, 'CSafeDumper', yaml.SafeDumper))`. This provides a ~5x speedup by seamlessly utilizing the C-based serialization when available, while safely falling back to pure Python without complex nested exception handling.
