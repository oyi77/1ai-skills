...
def select_posts_for_boost(max_posts: int = 5, hours_old_max: int = 24) -> list:
    ...
    for post in posts:
        ...
        published_at_str = post.get("published_at") or post.get("scheduled_at") or post.get("created_at")
        ...
        scored.append({
            ...
            "platforms": list(set(r.get("platform", "") for r in result_by_post.get(post_id, [])))
        })
...
def run_engagement_wave(
    ...
    _log(f"=== Engagement Wave | platform:{platform} | post:{post['post_id']} | accounts:{len(account_ids)} | actions:{actions} ===")
    ...
    for action in actions:
        if action == "like":
            ...
            if "comment" in actions and not dry_run:
                ...
                _log(f"Waiting {inter_action_delay:.0f}s between like and comment waves...")
...
def boost_latest_posts(
    ...
    _log(f"🚀 Boost Campaign START | posts:{max_posts} | platforms:{platforms} | dry_run:{dry_run}")
...
def print_status():
    ...
    try:
        resp = requests.get(f"{POSTBRIDGE_BASE}/social-accounts", headers=HEADERS, timeout=10)
...
def main():
    ...
    parser.add_argument("--boost-latest", action="store_true", help="Boost latest posts")
    parser.add_argument("--post-id", type=str, help="Boost specific post ID")
    parser.add_argument("--platform", type=str, default=None, help="Specific platform (tiktok/instagram/facebook)")
    parser.add_argument("--dry-run", action="store_true", help="Simulate without executing")
    parser.add_argument("--quick", action="store_true", help="Use short delays (testing)")
