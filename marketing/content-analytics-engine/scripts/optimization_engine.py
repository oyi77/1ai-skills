...
def score_platform(platform_data: dict) -> dict:
    ...
    max_views = max(
        (d["avg_views_per_post"] for d in platform_data.values()), default=1
    )
    max_er = max(
        (d["avg_engagement_rate"] for d in platform_data.values()), default=0.01
    )
...
def what_to_post_more(content_data: dict) -> list:
    ...
    sorted_types = sorted(
        content_data.items(), key=lambda x: x[1]["avg_views_per_post"], reverse=True
    )
    return [
        {
            "content_type": ctype,
            ...
            "posts": data["posts"],
            "verdict": (
                "POST MORE ✅" if i < 2 else "TEST MORE 🔄" if i < 4 else "POST LESS ❌"
            ),
        }
