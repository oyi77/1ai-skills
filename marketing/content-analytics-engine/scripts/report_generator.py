...
def generate_daily_report(dataset: dict, date: str = None) -> str:
    ...
    recommendations = generate_content_recommendations(dataset["analytics"], dataset["posts"])
    ...
    for stage, info in funnel["funnel_stages"].items():
        bar = "█" * min(int(info["count"] / max(funnel["raw_counts"]["views"], 1) * 20), 20)
    ...
    for p, t in trend["platform_trends"].items():
        ...
        lines.append(f"- **{p.upper()}:** {t.get('trend', 'n/a').upper()} {arrow} ({t.get('growth_pct', 0)}%)")
    ...
    for h in trend["timing"]["best_hours"]:
        lines.append(f"- {h['hour']}: {h.get('avg_views', 0)} avg views ({h.get('posts', 0)} posts)")
...
def generate_weekly_report(dataset: dict, week_start: str = None) -> str:
    ...
    for p, t in trend["platform_trends"].items():
        lines.append(f"- **{p.upper()}:** {t.get('trend', 'n/a')} ({t.get('growth_pct', 0)}%)")

    lines += [
        "",
        "## Key Insights",
        "1. " + (trend['growth_analysis']['trend'].upper() + " trend — " + (
            "Keep pushing!" if trend['growth_analysis']['trend'] == 'growing'
            else "Need to change strategy" if trend['growth_analysis']['trend'] == 'declining'
...
def save_report(content: str, report_type: str = "daily",
                date: str = None) -> str:
...
def save_json_output(data: dict, report_type: str = "daily",
                     date: str = None) -> str:
