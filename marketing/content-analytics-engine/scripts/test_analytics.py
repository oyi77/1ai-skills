...
class TestAnalyticsCollector(unittest.TestCase):
    ...
    def test_collect_all(self):
        ...
        print(f"  ✓ Full dataset: {sum(len(v) for v in [data['analytics'], data['posts'], data['post_results'], data['social_accounts']])} total records")
...
class TestPerformanceAnalyzer(unittest.TestCase):
    ...
    def test_engagement_rate(self):
        ...
        er = compute_engagement_rate({"view_count": 100, "like_count": 3,
...
class TestTrendDetector(unittest.TestCase):
    ...
    def test_weekly_trend(self):
        ...
        self.assertIn(trend["growth_analysis"]["trend"],
...
class TestOptimizationEngine(unittest.TestCase):
    ...
    def test_recommendations(self):
        ...
        recs = generate_content_recommendations(self.data["analytics"], self.data["posts"])
        self.assertIsInstance(recs, list)
        self.assertGreater(len(recs), 0)
        # Should have at least one CRITICAL recommendation given 0 sales
        priorities = [r["priority"] for r in recs]
        self.assertIn("CRITICAL", priorities)
        print(f"  ✓ {len(recs)} recommendations, {priorities.count('CRITICAL')} CRITICAL")
