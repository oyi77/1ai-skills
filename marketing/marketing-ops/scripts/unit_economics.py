...
def calculate_unit_economics(arpu, churn_rate, cac, expansion_rate=0):
    ...
    health = (
        ...
        else ("WARNING" if ltv_cac >= 1.5 else "CRITICAL")
    )
    ...
    return {
        ...
        "recommendations": get_recommendations(
            ltv_cac, payback_months, churn_rate, nrr
        ),
    }


def calculate_plg_funnel(
    visitors,
    signup_rate,
    activation_rate,
    conversion_rate,
    ...
    return {
        ...
        "to_reach_targets": {
            "$5k_mrr": (
                f"Need {5000/arpu:.0f} customers → {5000/arpu/conversion_rate*100/activation_rate*100/signup_rate*100:.0f} visitors/mo"
                if arpu > 0
                else "N/A"
            ),
...
def get_recommendations(ltv_cac, payback, churn, nrr):
    ...
    if ltv_cac < 1:
        recs.append(
            "CRITICAL: Losing money per customer. Reduce CAC or increase pricing immediately."
        )
    elif ltv_cac < 3:
        recs.append(
            "LTV:CAC below 3:1. Focus on reducing CAC (organic channels) or increasing LTV (pricing, upsells)."
        )
    else:
    ...
    if payback > 12:
        recs.append(
            "Payback >12 months. Consider annual billing discounts to accelerate cash recovery."
        )
    if churn > 8:
        recs.append(
            f"Churn at {churn}% is high. Prioritize retention before acquisition."
        )
    if nrr < 100:
        recs.append(
            "NRR below 100% — revenue is shrinking. Add expansion revenue (upsells, add-ons)."
        )
    return recs
...
def main():
    ...
    parser.add_argument(
        "--cac", type=float, default=0, help="Customer acquisition cost ($)"
    )
    parser.add_argument(
        "--expansion", type=float, default=0, help="Monthly expansion rate (%)"
    )
    parser.add_argument(
        "--visitors", type=float, default=0, help="Monthly visitors (PLG mode)"
    )
    parser.add_argument(
        "--signup-rate", type=float, default=0, help="Visitor to signup rate (%)"
    )
    parser.add_argument(
        "--activation", type=float, default=0, help="Signup to activated rate (%)"
    )
    parser.add_argument(
        "--conversion", type=float, default=0, help="Free to paid rate (%)"
    )
    parser.add_argument(
        "--spend", type=float, default=0, help="Monthly marketing spend ($)"
    )
